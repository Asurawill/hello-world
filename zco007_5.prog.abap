...
*&
*& Modify by     : IT02
*& Modify date   : 20152131
*& Request       :
*& Descriptions  :增加“直接执行（原报表展现逻辑）”、“后台执行”、"后台执行报表查询"
*&
*&---------------------------------------------------------------------*
REPORT ZCO007_5.
...
PARAMETERS:
           G_ZJZX TYPE CHAR1 RADIOBUTTON GROUP G1 USER-COMMAND UC DEFAULT 'X',      "直接执行
           G_HTZX TYPE CHAR1 RADIOBUTTON GROUP G1 ,                 "后台执行
           G_HTBBCX TYPE CHAR1 RADIOBUTTON GROUP G1.                  "后台执行报表查询

SELECTION-SCREEN END OF BLOCK BLK2.

SELECTION-SCREEN BEGIN OF BLOCK  BLK3 WITH FRAME TITLE  TEXT-003.
PARAMETERS:
           P_HTDATE LIKE SY-DATUM DEFAULT SY-DATUM MODIF ID HT,     "后台执行日期
           P_HTTIME LIKE SY-UZEIT DEFAULT SY-UZEIT MODIF ID HT.     "后台执行时间
...
*权限检查检查公司代码
 ...
 IF G_ZJZX EQ 'X'.
  ...
  ENDIF.
  IF G_HTZX EQ 'X'.
    PERFORM FRM_HTZX_DATA. "后台执行
   " PERFORM FRM_DEAL_DATA."处理数逻辑
  ENDIF.

  IF G_HTBBCX EQ 'X'.
    PERFORM FRM_SEL_HTCX. "取数逻辑
    PERFORM FRM_ALV_SHOW. "ALV显示
  ENDIF.
...
FORM FRM_INIT_LVC .
 IF G_HTBBCX EQ 'X'.
  INIT_FIELDCAT 'GZSTART'        '过账开始日期'             '' '' '' '' '' '' ''.
  INIT_FIELDCAT 'GZEND'          '过账结束日期'             '' '' '' '' '' '' ''.
 ENDIF.
  INIT_FIELDCAT 'WERKS'          '供货公司'             '' '' '' '' '' '' ''.
  INIT_FIELDCAT 'VBUND'          '采购公司'             '' '' '' '' '' '' ''.
  ...
  INIT_FIELDCAT 'CBCY1'          '期间成本差异'             '' '' '' '' '' '' ''.
  INIT_FIELDCAT 'SJCB'           '期间实际成本'             '' '' '' '' '' '' ''.
  INIT_FIELDCAT 'MENGE'          '期间交货数量'             '' '' '' '' '' '' ''.
  INIT_FIELDCAT 'QJSR'           '期间收入'             '' '' '' '' '' '' ''.
  INIT_FIELDCAT 'QJKP'           '期间开票数量'             '' '' '' '' '' '' ''.
  INIT_FIELDCAT 'QJYZ'           '期间应转工程成本'             '' '' '' '' '' '' ''.
  INIT_FIELDCAT 'REPOS'          '采购订单免费行'             '' '' '' '' '' '' ''.
  INIT_FIELDCAT 'WYSR'           '尾月收入'             '' '' '' '' '' '' ''.
  INIT_FIELDCAT 'WYKP'           '尾月开票数量'             '' '' '' '' '' '' ''.
  INIT_FIELDCAT 'WYFH'           '尾月发货数量'             '' '' '' '' '' '' ''.
  INIT_FIELDCAT 'WYCB'           '尾月确认成本'             '' '' '' '' '' '' ''.
...
* Date
  ...
  CONCATENATE  S_BUDAT-LOW+0(4) '年'
               S_BUDAT-LOW+4(2) '月' '到' S_BUDAT-HIGH+0(4) '年' S_BUDAT-HIGH+4(2) '月' INTO WA_HEADER-INFO.   "todays date
  APPEND WA_HEADER TO T_HEADER.
  CLEAR: WA_HEADER.
...
DATA: LV_JOB_NAME         LIKE TBTCO-JOBNAME,     "作业名
 ...
 DATA:MSG_OUT TYPE STRING.
 DATA:P_HTRQ TYPE C LENGTH 10,
      P_HTSJ TYPE C LENGTH 10.

    "如果是要传给PARAMETER  根据需求设置屏幕参数
*  LW_LINE-SELNAME = 'S_BUKRS'.
*  LW_LINE-KIND    = 'P'.
*  LW_LINE-SIGN    = 'I'.
*  LW_LINE-OPTION  = 'EQ'.
*  LW_LINE-LOW     = P_MONAT.
*  APPEND LW_LINE TO LT_RSPAR.
  ...
  LW_LINE-SIGN    = 'I'.
  LW_LINE-OPTION  = 'EQ'.
  LW_LINE-LOW     = S_BUKRS-LOW.
  APPEND LW_LINE TO LT_RSPAR.
  LW_LINE-SELNAME = 'S_BUDAT'.
  LW_LINE-KIND    = 'S'.
  LW_LINE-SIGN    = 'I'.
  LW_LINE-OPTION  = 'BT'.
  LW_LINE-LOW     = S_BUDAT-LOW.
  LW_LINE-HIGH    = S_BUDAT-HIGH.
  APPEND LW_LINE TO LT_RSPAR.
  "JOB 名称
  LV_JOB_NAME = 'ZCO007_5JOB' .
    ...
    IF SY-SUBRC = 0.
      ...
      ELSE.
         CLEAR :MSG_OUT,
                P_HTRQ,
                P_HTSJ.
        WRITE P_HTDATE TO P_HTRQ.
        WRITE P_HTTIME   TO P_HTSJ.
         MSG_OUT = '后台JOB已释放，并在'."始运行
        CONCATENATE MSG_OUT P_HTRQ '     ' P_HTSJ '运行' INTO MSG_OUT .
        MESSAGE MSG_OUT TYPE 'I'.
      ENDIF.
...
*----------------------------------------------------------------------*
FORM FRM_SEL_HTCX .
  SELECT * INTO CORRESPONDING FIELDS OF TABLE GT_ZCO007_5
    FROM ZCO007_5
    WHERE BUKRS = S_BUKRS-LOW
    AND   GZSTART = S_BUDAT-LOW.

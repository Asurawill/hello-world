...
* 只取347规格型号的物料
  ...
  " 期初库存( MARD和MARDH都要取，因为可能当前期间没有发生货物移动，MARD表还是存的之前期间的数据)
  SELECT MATNR
         WERKS
         LGORT
         LFGJA
  ...
  SORT GT_MARD_BEFORE BY MATNR WERKS LGORT ASCENDING LFGJA LFMON DESCENDING .
  SELECT MATNR
         WERKS
         LGORT
         LFGJA
  ...
  SORT GT_MARDH_BEFORE BY MATNR WERKS LGORT ASCENDING LFGJA LFMON DESCENDING .
  DELETE ADJACENT DUPLICATES FROM GT_MARDH_BEFORE COMPARING MATNR WERKS LGORT . " 只取最大期间的记录，删除剩下的，免得后面重复获取
  " 排除已经存在于MARD中的记录
  LOOP AT GT_MARDH_BEFORE INTO GS_MARDH_BEFORE .
    READ TABLE GT_MARD_BEFORE WITH KEY MATNR = GS_MARDH_BEFORE-MATNR
                                       WERKS = GS_MARDH_BEFORE-WERKS
                                       LGORT = GS_MARDH_BEFORE-LGORT BINARY SEARCH TRANSPORTING NO FIELDS .
    IF SY-SUBRC = 0 .
...
*     AND LFMON = P_LFMON .
  ...
  " 期初库存( MSKA和MKSAH都要取，因为可能当前期间没有发生货物移动，MSKA表还是存的之前期间的数据)
  SELECT MATNR
         WERKS
         LGORT
         VBELN
  ...
  SORT GT_MSKA_BEFORE BY MATNR WERKS LGORT VBELN POSNR ASCENDING LFGJA LFMON DESCENDING .
  SELECT MATNR
         WERKS
         LGORT
         VBELN
  ...
  SORT GT_MSKAH_BEFORE BY MATNR WERKS LGORT VBELN POSNR ASCENDING LFGJA LFMON DESCENDING .
  DELETE ADJACENT DUPLICATES FROM GT_MSKAH_BEFORE COMPARING MATNR WERKS LGORT VBELN POSNR. " 只取最大期间的记录，删除剩下的，免得后面重复获取
  " 排除已经存在于MSKA中的记录
  LOOP AT GT_MSKAH_BEFORE INTO GS_MSKAH_BEFORE .
    READ TABLE GT_MSKA_BEFORE WITH KEY MATNR = GS_MSKAH_BEFORE-MATNR
                                       WERKS = GS_MSKAH_BEFORE-WERKS
                                       LGORT = GS_MSKAH_BEFORE-LGORT
                                       VBELN = GS_MSKAH_BEFORE-VBELN
                                       POSNR = GS_MSKAH_BEFORE-POSNR BINARY SEARCH TRANSPORTING NO FIELDS .
...
*     AND LFMON = P_LFMON .
  ...
  " 期初库存( MSLB和MSLBH都要取，因为可能当前期间没有发生货物移动，MSLB表还是存的之前期间的数据)
  SELECT MATNR
         WERKS
         LIFNR
         SOBKZ
  ...
  SORT GT_MSLB_BEFORE BY MATNR WERKS LIFNR ASCENDING LFGJA LFMON DESCENDING .
  SELECT MATNR
         WERKS
         LIFNR
         SOBKZ
  ...
  SORT GT_MSLBH_BEFORE BY MATNR WERKS LIFNR ASCENDING LFGJA LFMON DESCENDING .
  DELETE ADJACENT DUPLICATES FROM GT_MSLBH_BEFORE COMPARING MATNR WERKS LIFNR. " 只取最大期间的记录，删除剩下的，免得后面重复获取
  " 排除已经存在于MSLB中的记录
  LOOP AT GT_MSLBH_BEFORE INTO GS_MSLBH_BEFORE .
    READ TABLE GT_MSLB_BEFORE WITH KEY MATNR = GS_MSLBH_BEFORE-MATNR
                                       WERKS = GS_MSLBH_BEFORE-WERKS
                                       LIFNR = GS_MSLBH_BEFORE-LIFNR BINARY SEARCH TRANSPORTING NO FIELDS .
    IF SY-SUBRC = 0 .

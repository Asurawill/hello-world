...
*    if gt_versn1-versn is not initial.
  ...
  LOOP AT gt_c_output.
    ...
    IF sy-subrc NE 0.
      SC_LEN = SC_LEN + 1. "统计导入该选项预算成功执行条数  add it02 20160229
...
*    if gt_versn1-versn is not initial.
  ...
  LOOP AT gt_cz_output.
    ...
    IF sy-subrc NE 0.
      SC_LEN = SC_LEN + 1. "统计导入该选项预算成功执行条数  add it02 20160229
      gt_cz_output-type = icon_green_light.
      CALL FUNCTION 'BAPI_TRANSACTION_COMMIT'
        EXPORTING
          wait = 'X'.
...
FORM frm_bapi_gt_hz_output .
  ...
  LOOP AT gt_hz_output.
    ...
    IF sy-subrc NE 0.
      ...
      gt_hz_output-type = icon_green_light.
...
FORM frm_bapi_gt_chz_output .
  ...
  LOOP AT gt_chz_output.
    IF gt_chz_output-posid IS NOT INITIAL.
    ...
    IF gt_chz_output-werks IS INITIAL.
      ls_item-resource-werks = 1800.                                  "工厂
      ls_item1-resource-werks = 1800.                                  "工厂
    ELSE.
      ls_item-resource-werks = gt_chz_output-werks.
      ls_item1-resource-werks = gt_chz_output-werks.
    ENDIF.

    IF gt_chz_output-xmlb = 'M'.
      IF gt_chz_output-matnr IS NOT INITIAL.
    ...
    IF gt_chz_output-matnr IS NOT INITIAL.
      SELECT meins
            FROM mara
            INTO TABLE gt_meins
            WHERE matnr = gt_chz_output-matnr.
    ENDIF.
    ...
    IF sy-subrc <> 0.
      ...
      READ TABLE gt_return1 INTO gs_return1 WITH KEY type = 'E'.
      IF sy-subrc <> 0 .
        gt_chz_output-type = icon_green_light.      "如果两个都更新成功，那么绿灯，而且0版本和100版本也会更新成功，只是消息会报目前两个是否成功
        ...
        CONCATENATE gs_return-message gs_return1-message INTO gt_chz_output-message.

      ELSE.
        gt_chz_output-type = icon_red_light.
        CONCATENATE gs_return-message gs_return1-message INTO gt_chz_output-message.

      ENDIF.
    ELSE.
      gt_chz_output-type = icon_red_light. "报出相应的错误，同时不更新0版本和100版本
      gt_chz_output-message = gs_return-message.

    ENDIF.

    MODIFY gt_chz_output TRANSPORTING hpeinh jpeinh type message.

    CLEAR:ls_item,ls_wbs_costlines-cost_lines,ls_wbs_costlines,gt_chz_output.
    REFRESH:lt_wbs_costlines,lt_wbs_costlines5,lt_wbs_costlines6.
    CLEAR:ls_item1,ls_wbs_costlines1-cost_lines,ls_wbs_costlines1.
    REFRESH lt_wbs_costlines1.
    CLEAR:gs_return,gs_return1.

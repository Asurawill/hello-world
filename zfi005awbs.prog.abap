...
*&---------------------------------------------------------------------*
*&---------------------------------------------------------------------*
*&modify by  it02 20160613
*&-that 履约保证金、其他押金、投标保证金的原因代码由”A01"改为 ‘A03”--------------------------------------------------------------------*
*&---------------------------------------------------------------------*
REPORT ZFI005.
...
DATA: L_PS_POSID1 TYPE BDCDATA-FVAL.
DATA: L_PROJK1   TYPE BDCDATA-FVAL.
DATA: L_ZUONR_012 TYPE BDCDATA-FVAL.
DATA: L_FVAL12 TYPE BDCDATA-FVAL.
DATA: L_SGTXT_013 TYPE BDCDATA-FVAL.
DATA: L_NEWBS_014 TYPE BDCDATA-FVAL.
DATA: L_HKONT_AKONT    TYPE  BDCDATA-FVAL."统驭科目
...
*        DATA:KMMS TYPE STRING."科目描述
*        DATA:XMMS TYPE STRING."WBS描述
...
*&--代码添加 BY HANDYBY 12.06.2017 21:33:44  BEGIN
DATA GS_SPLIT2 TYPE ZFI005_SPLIT .
*&--代码添加 BY HANDYBY 12.06.2017 21:33:44  END
...
DATA:BEGIN OF GT_PRPS OCCURS 1,
       "PSPNR TYPE PS_POSNR,
       POSID TYPE PS_POSID,
       POST1 TYPE PS_POST1,
     END OF GT_PRPS .
DATA:BEGIN OF GT_YWZL OCCURS 1,
       YWZL TYPE ZYWZL,
     END OF GT_YWZL .
DATA: GS_YWZL  LIKE LINE OF GT_YWZL.
"设置稳定刷新
DATA G_IS_STABLE TYPE LVC_S_STBL.
DATA L_STEP TYPE CHAR30.

DATA W_VBAK TYPE VBAK.
DATA:W_PRPS TYPE PRPS.
...
DATA:  GS_PRPS LIKE GT_PRPS.

DATA:GT_KNB1 LIKE TABLE OF KNB1 WITH HEADER LINE.

DATA:GT_SKAT_F4 LIKE TABLE OF SKAT WITH HEADER LINE.
DATA:YWZL_LIST  TYPE VRM_VALUES, "list box 的值列表  add 150817 it02
     YWZL_VALUE LIKE LINE OF YWZL_LIST. "list box的结构
...
START-OF-SELECTION.
  ...
  PERFORM INIDATA."查询科目 、PS WBS 信息
  ...
  IF R_PRGS[] IS NOT INITIAL.
    ...
    LOOP AT GT_ZFI005.
      ...
      ENDIF.
      "读取项目描述
      READ TABLE GT_PRPS WITH KEY POSID = GT_ZFI005-XMBH BINARY SEARCH.
      IF SY-SUBRC = 0 .
        GT_ZFI005-XMMS = GT_PRPS-POST1 .
      ENDIF.
      IF GT_ZFI005-SUB_COUNT IS INITIAL.
        GT_ZFI005-SUB_COUNT = 1.

      ENDIF.
      IF GT_ZFI005-AKONT EQ ''  . "为空取 客户默认统驭科目 ，反之按保存的科目显示
        READ TABLE GT_KNB1 WITH KEY KUNNR =  GT_ZFI005-KUNNR  BINARY SEARCH.
        IF SY-SUBRC = 0.
          GT_ZFI005-AKONT = GT_KNB1-AKONT.     "151106 APPEND IT02
        ENDIF.
...
*&--代码注释 BY HANDYBY 12.06.2017 20:55:22  BEGIN
*        SELECT * FROM GL_ACCT_CA_TEXT INTO CORRESPONDING FIELDS OF TABLE GT_SKA1[]
*          WHERE SPRAS EQ SY-LANGU
*          AND KTOPL EQ '1000'
*          AND SAKNR GE '1002010101'
*        AND SAKNR LE '1002999999'.
*&--代码注释 BY HANDYBY 12.06.2017 20:55:22  END
...
**  Inform ALV Grid that event 'onf4' has been processed
    ...
    ELSEIF E_FIELDNAME = 'XJKM'
          ...
          OR E_FIELDNAME = 'PJKM'
       "   OR E_FIELDNAME = 'AKONT' .
      .
      ER_EVENT_DATA->M_EVENT_HANDLED = 'X'.
    ENDIF.
    READ TABLE GT_ZFI005_EDIT INTO GW_ZFI005 INDEX ES_ROW_NO-ROW_ID.
    IF X_STEP2 EQ 'X' AND GW_ZFI005-STEP EQ ICON_YELLOW_LIGHT AND  E_FIELDNAME = 'YWZL'.
      IF E_FIELDNAME = 'YWZL'.   "添加业务种类选项 IT02 150817 原从域值取出 现更改为只填充两项数据 项目回款 、项目预收
        CLEAR GS_YWZL .
        CLEAR  GT_YWZL[].
        GS_YWZL-YWZL = '项目回款'.
        APPEND GS_YWZL TO GT_YWZL..
        GS_YWZL-YWZL = '项目预收'.
        APPEND GS_YWZL TO GT_YWZL..
        GS_YWZL-YWZL = '投标保证金'.
        APPEND GS_YWZL TO GT_YWZL..
...
**  Update the value in ALV cell
          ...
          ENDIF.
          ER_EVENT_DATA->M_EVENT_HANDLED = 'X'.
*&--代码添加 BY HANDYBY 12.06.2017 21:22:05  BEGIN
          READ TABLE GT_SPLIT INTO GS_SPLIT2 INDEX ES_ROW_NO-ROW_ID .
          IF SY-SUBRC = 0 .
            GS_SPLIT2-YWZL = LW_RET_TAB-FIELDVAL .
            MODIFY GT_SPLIT FROM GS_SPLIT2 INDEX ES_ROW_NO-ROW_ID .
          ENDIF.
*&--代码添加 BY HANDYBY 12.06.2017 21:22:05  END
        ENDIF.
      ENDIF.
...
**  Update the value in ALV cell
    ...
    " IF X_STEP3 EQ 'X'. "  根据客户选择统驭科目

*         IF E_FIELDNAME = 'AKONT'.
*            DELETE GT_KNB1 WHERE KUNNR NE GW_ZFI005-KUNNR.
*            SELECT SAKNR TXT50
*              INTO CORRESPONDING FIELDS OF TABLE GT_SKAT_F4
*               FROM SKAT  WHERE SPRAS = '1' AND KTOPL = '1000'.
*            CALL FUNCTION 'F4IF_INT_TABLE_VALUE_REQUEST'
*                EXPORTING
*                 retfield        = 'AKONT' "大写,可选值内表的字段名
*                 value_org       = 'S' "就写'S'
...
*  ENDMETHOD.
  ...
  METHOD
    ...
    IF SY-SUBRC EQ 0.
      CASE E_COLUMN.
        WHEN 'XMBH'."项目编号（WBS元素号）
          IF GW_ZFI005-XMBH IS NOT INITIAL.
            SET PARAMETER ID  'PRO' FIELD GW_ZFI005-XMBH.
        ...
        LOOP AT LT_ROWS INTO LW_ROWS.
          ...
          IF SY-SUBRC EQ 0.
            IF GW_ZFI005-STEP NE ICON_YELLOW_LIGHT OR GW_ZFI005-DEL_ICON NE SPACE.
              ...
              RETURN.
            ENDIF.
            IF GW_ZFI005-AKONT EQ '' .
              MESSAGE '请输入统驭科目！' TYPE 'S' DISPLAY LIKE 'E'.
            ENDIF.
...
*             IF GW_ZFI005-AKONT EQ '1122010101'.
*              CLEAR L_GL.
*            ENDIF.
            IF GW_ZFI005-AKONT EQ  '1122020101'  . "如果统驭科目为1122020101 则为关联，否则为 非关联
              L_GL = 'X'."关联
            ELSE.
              CLEAR L_GL. "非关联
            ENDIF.
...
*             L_NEWUM_008 = '*'.
              ...
              L_ZUONR_012 = GW_ZFI005-XMBH."add 分配1 it02 150818
              L_PS_POSID1 = GW_ZFI005-XMBH. "add wbs it02 150816
...
*             L_HKONT_021 = '1122010101'.
              ...
              CASE GW_ZFI005-YWZL.
                WHEN '项目回款'.
                  ...
                  L_NEWUM_008 = ''."第二行SGL标识
                  L_NEWBS_014 = '11' ."第二行记帐码
                  L_RSTGR_018 = 'A01'. "原因代码
                  IF GW_ZFI005-AKONT NE ''.
                    L_HKONT_AKONT     = GW_ZFI005-AKONT .  "统驭科目
                  ENDIF.
                ...
                WHEN '项目预收'.
                  ...
                  L_NEWBS_014 = '19' ."第二行记帐码
                  L_RSTGR_018 = 'A01'. "原因代码
                  "       L_PS_POSID2 = GW_ZFI005-XMBH. "add wbs it02 150816
                WHEN '投标保证金'.
                  L_NEWBS_009 = '40'."第一行记帐码
                  L_NEWUM_008 = ''."第二行SGL标识
                  L_NEWBS_014 = '11' ."第二行记帐码
                  L_RSTGR_018 = 'A03'. "原因代码
                  IF GW_ZFI005-AKONT NE ''.
                    L_HKONT_AKONT     = GW_ZFI005-AKONT .  "统驭科目
                  ENDIF.
                WHEN '履约保证金'.
...
*                  L_NEWBS_014 = '11' ."第二行记帐码
*                ELSE. "非关联
*                  L_NEWBS_009 = '40'."第一行记帐码    " WBS 没有 维修回款 、 维修预收项
...
*                ENDIF.
              ...
              CONDENSE: L_HKONT_021,L_WRBTR_022,L_ZFBDT_023,L_ZUONR_024,L_SGTXT_025 ,L_BKTXT,L_PS_POSID1,L_PS_POSID2,L_ZUONR_012.
              PERFORM EXPORT_SPLIT1 USING L_GL.

              CALL FUNCTION 'ZFI005_1_SPLITWBS'
                EXPORTING
*                 CTU         = 'X'
                  MODE        = 'N'
*                 MODE        = 'E'
...
*                 DOCID_008   = L_NEWUM_008
                  ...
                  HKONT_AKONT = L_HKONT_AKONT   "统驭科目
                  WRBTR_011   = L_WRBTR_011
                  PS_POSID1   = L_PS_POSID1
*                 PROJK1      = L_PROJK1
                  "ZUONR_012       = '' "项目编号
                  ZUONR_012   = L_ZUONR_012  "项目编号
...
*              L_FVAL08 = '*'.
              ...
              L_WBANK = GW_ZFI005-PJBH."票据编号
              "   L_ZUONR_013 = GW_ZFI005-PJBH."票据编号
              L_ZUONR_013 = GW_ZFI005-XMBH."分配字段填充 WBS元素
              L_NEWBS_020 = '11'.
*              L_FVAL25 = ''.
...
*              L_HKONT_022 = GW_ZFI005-PJKM.
              ...
              CASE GW_ZFI005-YWZL.
                ...
                WHEN '投标保证金'.
                  ...
                  L_NEWBS_020 = '11'."第二行记帐码
                  L_NEWUM_027 = ''."第二行SGL标识
                  L_HKONT_AKONT = '1221010101' . "第二行科目其他应收款-非关联方-投标保证金
                WHEN '履约保证金'.
                  L_NEWBS_009 = '09'."第一行记帐码
                  L_NEWUM_011 = 'W'."第一行SGL标识
                  L_NEWBS_020 = '11'."第二行记帐码
                  L_NEWUM_027 = ''."第二行SGL标识
                  L_HKONT_AKONT = '1221010201' . "第二行科目其他应收款-非关联方-履约保证金
                WHEN '其他押金'.
                  L_NEWBS_009 = '09'."第一行记帐码
                  L_NEWUM_011 = 'W'."第一行SGL标识
                  L_NEWBS_020 = '11'."第二行记帐码
                  L_NEWUM_027 = ''."第二行SGL标识
                  L_HKONT_AKONT = '1221010601' . "第二行科目其他应收款-非关联方-其他押金
...
*                 ENDIF.
              ...
              CALL FUNCTION 'ZFI005_3_SPLITWBS'
                EXPORTING
*                 CTU         = 'X'
                  MODE        = 'N'
*                 UPDATE      = 'L'
...
*               FROM BKPF
*               WHERE BUKRS = GW_ZFI005-BUKRS
*               AND BELNR = GW_ZFI005-BELNR
*               AND GJAHR = GW_ZFI005-BUDAT+0(4).
*              CALL FUNCTION 'CONVERSION_EXIT_ALPHA_INPUT'
*                EXPORTING
*                 input  = GW_ZFI005-BELNR
*                IMPORTING
*                 output = GW_ZFI005-BELNR.
...
*&--代码添加 BY HANDYBY 13.06.2017 18:06:36  END
      ...
      WHEN 'SPLIT'."拆行
        ...
        IF SY-SUBRC EQ 0.
          ...
          MOVE-CORRESPONDING GW_ZFI005 TO GS_SPLITPARENT . " GS_SPLITPARENT.
...
*            IF GT_SPLIT-KUNNR IS INITIAL.                 “WBS收款暂不需要 与客户直接关联，客户直接手工选择即可
...
*    MESSAGE S000(ZFICO01).
  ...
  METHOD
    ...
    IF E_MODIFIED EQ 'X'.
      ...
      ENDLOOP.
      "设置项目WBS描述
      LOOP AT ET_GOOD_CELLS INTO LW_CELL WHERE FIELDNAME EQ 'XMBH'."设置项目编号
        READ TABLE GT_ZFI005_EDIT INTO GW_ZFI005 INDEX LW_CELL-ROW_ID.
        READ TABLE GT_PRPS INTO GS_PRPS WITH KEY POSID = LW_CELL-VALUE BINARY SEARCH.
        IF SY-SUBRC = 0.
          GW_ZFI005-XMMS = GS_PRPS-POST1.
        ENDIF.
        MODIFY GT_ZFI005_EDIT FROM GW_ZFI005 INDEX LW_CELL-ROW_ID.
      ENDLOOP.
      "根据项目号设置客户号和客户名称
*      LOOP AT ET_GOOD_CELLS INTO LW_CELL WHERE FIELDNAME EQ 'XMBH'."设置客户号(收票方)
*        READ TABLE GT_ZFI005_EDIT INTO GW_ZFI005 INDEX LW_CELL-ROW_ID.       ”WBS收款暂不需要根据WBS自动带出客户 ，客户有手动选择即可 “150818
...
*      LOOP AT ET_GOOD_CELLS INTO LW_CELL WHERE FIELDNAME EQ 'XMBH'."设置客户号(收票方)
*        READ TABLE GT_SPLIT INTO GT_SPLIT INDEX LW_CELL-ROW_ID.
*        DATA L_KUNNR TYPE VBPA-KUNNR.                                "WBS收款平台暂不需要根据WBS关联客户，客户有手工选择即可
...
*      DATA: LT_F4 TYPE LVC_T_F4 WITH HEADER LINE.
    ...
    APPEND LT_F4.
    LT_F4-FIELDNAME = 'YWZL'.
    APPEND LT_F4.  " 业务种类增加搜索帮助 150817 it02
...
*   ENDIF.
  "保存时应先删掉销售回款编号为空的数据
  " DELETE FROM  ZFI005 WHERE BUKRS = P_BUKRS AND XSHKNO IS  NULL .
...
*  'PJKM' ' 票据科目' '' '1' 'BSEG' 'HKONT' '',
  ...
  'KUNNR' '客户编号' '' '2' '' '' '',
  'KUNNRT' '客户描述' '' '' '' '' '',
  'AKONT' '统驭科目' '' '3' 'BSEG' 'HKONT' '',
  'XMBH' 'WBS元素' '' '2' 'PRPS' 'POSID' '',
  'XMMS' 'WBS元素描述' '' '' '' '' '',
  'GZCKH' '过账参考号' '' '' '' '' '',
  'BELNR' '过账会计凭证编号' '' '' '' '' '',
  'BUDAT' '过账日期' '' '' '' '' '',
...
FORM ADD_FIELDCAT USING FIELDNAME TEXT CFIELDNAME EDIT REF_TABLE REF_FIELD CHECKBOX.
  ...
  IF FIELDNAME EQ 'XJKM'
    ...
    OR FIELDNAME EQ 'YWZL' .
    "  OR FIELDNAME EQ 'AKONT'. "IT02 150817
...
*            RETURN.
*          ENDIF.
            SELECT SINGLE * FROM PRPS INTO W_PRPS WHERE POSID EQ GW_ZFI005-XMBH.
            IF SY-SUBRC NE 0.
              CALL FUNCTION 'CONVERSION_EXIT_ALPHA_OUTPUT'
                EXPORTING
                  INPUT  = GW_ZFI005-XMBH      "由销售订单号替换为WBS元素 IT02 150807
                IMPORTING
                  OUTPUT = L_XMBH.
              MESSAGE S899(MM) WITH 'WBS元素' L_XMBH '不存在' DISPLAY LIKE 'E'.
              P_ERR = 'X'.
              RETURN.
            ENDIF.
          ENDIF.
...
*        RETURN.
*      ENDIF.
      SELECT SINGLE * FROM PRPS INTO W_PRPS WHERE POSID EQ GT_SPLIT-XMBH.
      IF SY-SUBRC NE 0.
        CALL FUNCTION 'CONVERSION_EXIT_ALPHA_OUTPUT'
          EXPORTING
            INPUT  = GW_ZFI005-XMBH      "由销售订单号替换为WBS元素 IT02 150807
        ...
        RETURN.
      ENDIF.
*&--代码添加 BY HANDYBY 12.06.2017 21:56:40  BEGIN
    ELSE.
      MESSAGE 'WBS元素不能为空！' TYPE 'E' .
      P_ERR = 'X'.
      RETURN .
*&--代码添加 BY HANDYBY 12.06.2017 21:56:40  END
    ENDIF.
*&--代码添加 BY HANDYBY 12.06.2017 21:56:55  BEGIN
    ...
    IF GT_SPLIT-kunnr IS INITIAL .
      ...
      RETURN .
    ENDIF.
*&--代码添加 BY HANDYBY 12.06.2017 21:56:55  END
...
*       T_ITEM_SPLIT1-PS_POSID2 = L_PS_POSID2. "add wbs it02 150816
*     WHEN '维修回款'.
*       IF L_GL EQ 'X'.
*         T_ITEM_SPLIT1-NEWUM_008 = ''."第二行SGL标识
*         T_ITEM_SPLIT1-NEWBS_014 = '11' ."第二行记帐码
*       ELSE.                                                "WBS平台只有 项目回款及项目预收项
...
*    IF L_PS_POSID2 IS NOT INITIAL.
*    T_ITEM_SPLIT1-PS_POSID2 = L_PS_POSID2.
*    ENDIF.
    ...
    CLEAR T_ITEM_SPLIT1.
  ENDIF.
  EXPORT T_ITEM_SPLIT1[] TO MEMORY ID 'ZFI005_1_ITEMWBS'.
ENDFORM.
*&---------------------------------------------------------------------*
*&      Form  EXPORT_SPLIT3
*&---------------------------------------------------------------------*
  ...
  SELECT * FROM ZFI005_SPLIT INTO CORRESPONDING FIELDS OF GT_SPLIT
    ...
    CASE GT_SPLIT-YWZL.
      ...
      WHEN '项目预收'.
        ...
        "    T_ITEM_SPLIT3-PS_POSID2 = L_PS_POSID2. "add wbs it02 150816
*      WHEN '维修回款'.
*        IF L_GL EQ 'X'.
*         T_ITEM_SPLIT3-NEWBS_020 = '11'."第二行记帐码
*         T_ITEM_SPLIT3-NEWUM_027 = ''."第二行SGL标识
*       ELSE.
*         T_ITEM_SPLIT3-NEWBS_020 = '19'."第二行记帐码
*         T_ITEM_SPLIT3-NEWUM_027 = 'T'."第二行SGL标识  "WBS项 无 维修回款 、维修预收 项目
*       ENDIF.
*      WHEN '维修预收'.
*        IF L_GL EQ 'X'.
*         T_ITEM_SPLIT3-NEWBS_020 = '19'."第二行记帐码
...
*       IF L_PS_POSID2 IS NOT INITIAL.
*     T_ITEM_SPLIT3-PS_POSID2 = L_PS_POSID2.
*  ENDIF.
    ...
    CLEAR T_ITEM_SPLIT3.
  ENDIF.
  EXPORT T_ITEM_SPLIT3[] TO MEMORY ID 'ZFI005_3_ITEMWBS'.
ENDFORM.
...
FORM INIDATA .
  ...
  CLEAR GT_PRPS.
  SELECT  POSID POST1 FROM PRPS
  INTO CORRESPONDING FIELDS OF TABLE GT_PRPS.
  SORT GT_PRPS BY POSID.
  SELECT * INTO CORRESPONDING FIELDS OF TABLE GT_KNB1 FROM KNB1 WHERE BUKRS = P_BUKRS.

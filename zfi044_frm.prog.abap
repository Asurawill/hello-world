...
FORM FRM_GET_AND_PROCESS_DATA .
  ...
  " 项目类型
  DATA: LT_TCJ1T TYPE TABLE OF TCJ1T,
        LS_TCJ1T TYPE TCJ1T.
    ...
    IF LT_COEPB IS NOT INITIAL .
      ...
      SELECT KOKRS
       ...
       WHERE OBJNR EQ LT_PRPS-OBJNR
         AND ( ( GJAHR EQ P_DATUM+0(4) AND PERIO LE P_DATUM+4(2) )
    ...
    IF SY-SUBRC EQ 0 .
      LOOP AT LT_PRPS INTO LS_PRPS FROM SY-TABIX .
        IF LS_PRPS-PSPHI EQ LS_PROJ-PSPNR .
          ...
          READ TABLE LT_TCJ1T INTO LS_TCJ1T WITH KEY PRART = LS_PRPS-PRART BINARY SEARCH .
          GS_ALV-PRATX = LS_TCJ1T-PRATX .
          " 后面汇总用
          IF LS_PRPS-STUFE = 1 .
          ...
          READ TABLE LT_BKSG2 WITH KEY PROJK = LS_PRPS-PSPNR BINARY SEARCH TRANSPORTING NO FIELDS .
          IF SY-SUBRC = 0 .
            LOOP AT LT_BKSG2 INTO LS_BKSG2 FROM SY-TABIX .
              IF LS_BKSG2-PROJK = LS_PRPS-PSPNR .
          ...
          SORT LT_BKSG3 BY BUKRS BELNR GJAHR BUZEI .
          DELETE ADJACENT DUPLICATES FROM LT_BKSG3 COMPARING BUKRS BELNR GJAHR BUZEI .
          ...
          LOOP AT LT_BKSG3 INTO LS_BKSG3 .
            ...
            IF GS_ALV-YSCB EQ 0 .
              GS_ALV-WGBFB = 100 .
            ELSE .
              GS_ALV-WGBFB = GS_ALV-GCSG / GS_ALV-YSCB * 100 .
            ...
            IF LS_BKSG3-HKONT EQ '2221030501' .
              ...
              IF LS_BKSG3-SHKZG = 'S'.
                IF L_MWSKZ IS NOT INITIAL AND L_MWSKZ NE 0 .
...
* 小计
    ...
    READ TABLE LT_TCJ1T INTO LS_TCJ1T WITH KEY PRART = L_PRART BINARY SEARCH .
    GS_ALV-PRATX = LS_TCJ1T-PRATX .
    ...
    ENDIF.
    READ TABLE LT_KNA1 INTO LS_KNA1 WITH KEY KUNNR = GS_ALV-ZKHBM BINARY SEARCH .
    GS_ALV-NAME1 = LS_KNA1-NAME1 .
...
*    GS_ALV-WGBFB = L_WGBFB .
    ...
    GS_ALV-CLCG = L_CLCG .
    GS_ALV-FBCG = L_FBCG .
    GS_ALV-WSKJE = GS_ALV-ZHTJR - L_LJSK ."L_WSKJE .
    " 设置行颜色
    GS_ALV-CLR = 'C310' .
...
FORM FRM_CHECK_EMPTY .
  IF P_BUKRS IS INITIAL ..
    MESSAGE '公司代码不能为空' TYPE 'S' DISPLAY LIKE 'E' .
    LEAVE LIST-PROCESSING .
  ENDIF.
ENDFORM.

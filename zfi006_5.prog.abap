...
*处理进度默认为3
      ...
      GS_DATA-ZFKXZ = '3'.
      "审批人默认为申请人
      GS_DATA-ZNAME1 = SY-UNAME.

*默认货币，申请日期,申请付款日期
      GS_DATA-ZSQRQ      = SY-DATUM.
      GS_DATA-ZSQFKRQ    = SY-DATUM.
...
*可申请金额需要减去已付金额
      ...
      IF SY-SUBRC = 0.
        ...
        "    GS_DATA-ZSQFKJE = GS_RSEG_1-WRBTR  -  GS_DATA-YSQJE_AFP -  GS_DATA-YSQJE_TZFP . "默认申请付款金额
      ENDIF.
...
*&--代码添加 BY HANDYBY 26.06.2017 10:22:41  BEGIN
  INIT_FIELDCAT 'SE_BB'          '税额（本币）'   '' '' '' '' '' '' ''.
  INIT_FIELDCAT 'SE'        '税额'   '' '' '' '' '' '' ''.
*&--代码添加 BY HANDYBY 26.06.2017 10:22:41  END
  ...
  INIT_FIELDCAT 'ZSQRQ'          '申请日期'   '' '' '' '' '' 'ZFI006' 'ZSQRQ'.
...
*检查抬头
  IF S_EBELN IS NOT INITIAL .
    ...
    WHERE ZBOX = 'X'.
      IF GS_DATA-ZSQFKJE = 0.
        MESSAGE '申请付款金额不能为0,请更改!'  TYPE 'S' DISPLAY LIKE 'E'.
        L_SUBRC = 4.
      ENDIF.
      ...
      IF GS_DATA-ZZY IS INITIAL .
        MESSAGE '摘要不能为空' TYPE 'S' DISPLAY LIKE 'E'.
        L_SUBRC = 4.
      ENDIF.
...
FORM FRM_PRINT .
  ...
  DATA:L_FORMNAME TYPE TDSFNAME VALUE 'ZSFFI006_5'.
  DATA:L_PAGE  TYPE I.

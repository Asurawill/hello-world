...
FORM FRM_GET_DATA .
  ...
  SORT GT_PROJ BY PSPNR.
   ...
   SORT GT_PR BY  BANFN BNFPO MATNR.
...
FORM FRM_INIT_LVC .
 ...
 INIT_FIELDCAT 'TD_MENGE'       '折算数'         '' '' '' 'X' '' 'EBAN' 'MENGE'.
 ELSE.
 INIT_FIELDCAT 'BANFN'        '采购申请编号'         '' '' '' '' '' '' ''.
 INIT_FIELDCAT 'BNFPO'        '采购申请行项目'         '' '' '' '' '' '' ''.
 INIT_FIELDCAT 'FRGKZ'         '处理状态'         '' '' '' '' '' '' ''.
...
*         DIAGNOSE_OBJECT             = ' '
         ...
         IF SY-SUBRC EQ 0 .
          "  SORT GT_DATA BY BANFN BNFPO .
            LOOP AT GT_DATA INTO GS_DATA WHERE SEL = 'X' .
                 LOOP AT GT_DATA INTO GS_DATA WHERE  BANFN =  GS_DATA-BANFN
          ...
          ELSE.
            ...
            MESSAGE '请选择要保存的相应采购申请替代记录保存'  TYPE 'S'   DISPLAY LIKE 'E'.
      ...
      CHECK GS_DATA_SEL IS NOT INITIAL.
       ...
       SORT GT_PROJ_SEL BY BANFN  PSPID.
       DELETE ADJACENT DUPLICATES FROM GT_PROJ_SEL COMPARING BANFN  PSPID .
        "先删除已选择物理凭证对应的记录台账信息
       LOOP AT GT_PROJ_SEL INTO GS_PROJ_SEL.

          "根据选择采购申请号、项目号维度区分 删除已维护台账信息表
         DELETE FROM ZTDTZ_PR WHERE BANFN = GS_PROJ_SEL-BANFN
     ...
     CHECK L_SUBRC EQ 0.
      LOOP AT GT_DATA_SEL INTO GS_DATA_SEL .
        ...
        CLEAR GS_ZTDTZ_PR .
        MOVE-CORRESPONDING GS_DATA_SEL TO GS_ZTDTZ_PR.

        "维护账号
        GS_ZTDTZ_PR-UNAME = SY-UNAME.
        "维护日期
        GS_ZTDTZ_PR-UDATE = SY-DATUM.

        "维护时间
        GS_ZTDTZ_PR-UTIME = SY-UZEIT .

        APPEND GS_ZTDTZ_PR TO GT_ZTDTZ_PR.
      ENDLOOP.


      SORT GT_ZTDTZ_PR BY BANFN BNFPO .
       CHECK GT_ZTDTZ_PR IS NOT INITIAL.
      "保存数据到 ZTDTZ_PR.
       MODIFY ZTDTZ_PR FROM TABLE GT_ZTDTZ_PR .

           "先删除已选择物理凭证对应的记录台账信息
       LOOP AT GT_PROJ_SEL INTO GS_PROJ_SEL.

          "根据选择采购申请号、项目号维度区分 删除已且替代组且替代组标识且折算数量这三项都 为空的记录维护台账信息表
         DELETE FROM ZTDTZ_PR WHERE BANFN = GS_PROJ_SEL-BANFN
                              AND PSPID = GS_PROJ_SEL-PSPID
                              AND  TD_MATNR EQ ''
                              AND TD_GRPFLAG EQ ''
...
*      MESSAGE '被替代物料不能为空'  TYPE 'S' DISPLAY LIKE 'E'.
*         L_SUBRC = 4.
*         EXIT.
*    ENDIF.

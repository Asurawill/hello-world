...
TABLES: T001,T001W,MARA,MARD,MBEW,T030.
"MB5B获取数据用类型定义
...
DATA: GT_ALV_SUM TYPE STANDARD TABLE OF TY_ALV,
      GW_ALV_SUM LIKE LINE OF GT_ALV_SUM.
"明细展示用ALV内表
...
TYPES: BEGIN OF TY_SUM_DATA,
         ...
         MVGRP(2) TYPE N, "ZZT_MVGP-MVGRP,"HANDY
...
DATA: GT_SUM_MSEG TYPE STANDARD TABLE OF TY_SUM_DATA,
      GW_SUM_MSEG LIKE LINE OF GT_SUM_MSEG.
...
*
*DATA: GT_ZMVGRP TYPE STANDARD TABLE OF ZTMVGRP, "HANDWY
...
*DATA: LT_ZFI035T TYPE STANDARD TABLE OF ZFI035T,"BY HANDWY
...
TYPES: BEGIN OF TY_SUM_MBLNR,
         MATNR   TYPE MARA-MATNR,
         WERKS   TYPE T001W-WERKS,
         SIMNG   TYPE MSEG-MENGE,
         SOMNG   TYPE MSEG-MENGE,
         BMENG   TYPE MSEG-MENGE,
         BDMBT   TYPE MSEG-DMBTR,
         EMENG   TYPE MSEG-MENGE,
         EDMBT   TYPE MSEG-DMBTR,
         DMBTR_S TYPE MSEG-DMBTR,
         STDAT   TYPE ABPER_RF,
         EDDAT   TYPE ABPER_RF,
         CONTA   TYPE N,
         CONTB   TYPE N,
...
DATA: GT_SUM_DMBTR TYPE STANDARD TABLE OF TY_SUM_MBLNR,
      GW_SUM_DMBTR LIKE LINE OF GT_SUM_DMBTR.
...
DATA:I_VALUTAB LIKE RSPARAMS OCCURS 0 WITH HEADER LINE,
     ...
     I_DESC    LIKE VARID,
     T_VAL     LIKE RSPARAMS OCCURS 0 WITH HEADER LINE,
     T_RPMAX   LIKE RSPARAMS OCCURS 0 WITH HEADER LINE.
...
FORM FRM_IMPORT_DATA .
  ...
  DATA: L_TABIX    LIKE SY-TABIX,
        ...
        LC_TABIX_E LIKE SY-TABIX.
*        L_ZKGSB    TYPE ZZT_KCYDZ-ZKGSB. HANDWY
...
FORM FRM_DEAL_DATA.
  ...
  SORT GT_ZFI035C BY MATNR BUKRS WERKS LGORT CHARG EDDAT.
  SORT GT_MBEW BY MATNR BWKEY BWTAR.

  CLEAR: GT_SUM_DATA,GT_SUM_BELNR,GT_SUM_MBLNR,GT_SUM_MSEG.


  "对于没有移动类型的数据，需要进行分摊，先进行汇总；
  "其他数据需要根据移动组汇总
...
*    GW_SUM_DATA-SOBKZ = GW_ZFI035P-SOBKZ. "特殊库存标识
    ...
    IF GW_ZFI035P-ZMSEG <> 'X'.
      COLLECT GW_SUM_DATA INTO GT_SUM_MSEG.
    ENDIF.
    "******************************************************END
  ENDLOOP.
  CLEAR GT_ALV.
  SORT GT_SUM_MSEG BY MATNR WERKS LGORT CHARG.
  "针对当前期初库存不为0的数据放到ALV中
  LOOP AT GT_ZFI035C INTO GW_ZFI035C WHERE STDAT = P_STDAT.
    CLEAR GW_ALV.
    ...
    "期末库存
    READ TABLE GT_ZFI035C INTO GW_ZFI035C_TMP WITH KEY
    MATNR = GW_ALV-MATNR
    BUKRS = GW_ALV-BUKRS
    WERKS = GW_ALV-WERKS
    LGORT = GW_ALV-LGORT
    CHARG = GW_ALV-CHARG
    EDDAT = P_EDDAT BINARY SEARCH.
    IF SY-SUBRC = 0.
      GW_ALV-EMENG = GW_ZFI035C_TMP-MENGE.
      GW_ALV-EDMBT = GW_ZFI035C_TMP-DMBTR.
    ENDIF.
    ...
    IF SY-SUBRC = 0.
      GW_ALV-MATKL = GW_ZFI035K-MATKL.
      GW_ALV-BKLAS = GW_ZFI035K-BKLAS.
    ENDIF.

    LOOP AT GT_SUM_MSEG INTO GW_SUM_MSEG
      ...
      L_TABIX_X = SY-TABIX.
      IF GW_SUM_MSEG-MVGRP = '11' OR GW_SUM_MSEG-MVGRP = '21' OR GW_SUM_MSEG-MVGRP = '52'  "HANDWY
      OR GW_SUM_MSEG-MVGRP = '62' OR GW_SUM_MSEG-MVGRP = '42' OR GW_SUM_MSEG-MVGRP = '43'
      OR GW_SUM_MSEG-MVGRP = '54' OR GW_SUM_MSEG-MVGRP = '23' OR GW_SUM_MSEG-MVGRP = '41'
      OR GW_SUM_MSEG-MVGRP = '33' OR GW_SUM_MSEG-MVGRP = '61' OR GW_SUM_MSEG-MVGRP = '23'.
        GW_ALV-SIMNG_X = GW_ALV-SIMNG_X + GW_SUM_MSEG-MENGE.
      ELSEIF GW_SUM_MSEG-MVGRP = '31' OR GW_SUM_MSEG-MVGRP = '32' OR GW_SUM_MSEG-MVGRP = '51'
          OR GW_SUM_MSEG-MVGRP = '53' OR GW_SUM_MSEG-MVGRP = '63' OR GW_SUM_MSEG-MVGRP = '24'
          OR GW_SUM_MSEG-MVGRP = '22' OR GW_SUM_MSEG-MVGRP = '55' OR GW_SUM_MSEG-MVGRP = '34'
          OR GW_SUM_MSEG-MVGRP = '62' OR GW_SUM_MSEG-MVGRP = '20'.
        GW_ALV-SOMNG_X = GW_ALV-SOMNG_X + GW_SUM_MSEG-MENGE.
      ENDIF.
      DELETE GT_SUM_MSEG INDEX L_TABIX_X.
    ENDLOOP.
    ...
    LOOP AT GT_SUM_DATA INTO GW_SUM_DATA
      ...
      CASE GW_SUM_DATA-MVGRP.   "HANDWY
        WHEN '11'.
          ...
          IF GW_ALV-IMMNG IS NOT INITIAL.
            GW_ALV-IMAVP = GW_ALV-IMDMB / GW_ALV-IMMNG.
          ENDIF.
        WHEN '21'.
          GW_ALV-IDMNG = GW_SUM_DATA-MENGE.
          GW_ALV-IDDMB = GW_SUM_DATA-DMBTR.
          IF GW_ALV-IDMNG IS NOT INITIAL.
            GW_ALV-IDAVP = GW_ALV-IDDMB / GW_ALV-IDMNG.
          ENDIF.
        WHEN '52'.
          GW_ALV-IWMNG = GW_SUM_DATA-MENGE.
          GW_ALV-IWDMB = GW_SUM_DATA-DMBTR.
          IF GW_ALV-IWMNG IS NOT INITIAL.
            GW_ALV-IWAVP = GW_ALV-IWDMB / GW_ALV-IWMNG.
          ENDIF.
        WHEN '62'.
          GW_ALV-IPMNG = GW_SUM_DATA-MENGE.
          GW_ALV-IPDMB = GW_SUM_DATA-DMBTR.
          IF GW_ALV-IPMNG IS NOT INITIAL.
            GW_ALV-IPAVP = GW_ALV-IPDMB / GW_ALV-IPMNG.
          ENDIF.
        WHEN '42'.
          GW_ALV-IHMNG = GW_SUM_DATA-MENGE.
          GW_ALV-IHDMB = GW_SUM_DATA-DMBTR.
          IF GW_ALV-IHMNG IS NOT INITIAL.
            GW_ALV-IHAVP = GW_ALV-IHDMB / GW_ALV-IHMNG.
          ENDIF.
        WHEN '43'.
          GW_ALV-ICMNG = GW_SUM_DATA-MENGE.
          GW_ALV-ICDMB = GW_SUM_DATA-DMBTR.
          IF GW_ALV-ICMNG IS NOT INITIAL.
            GW_ALV-ICAVP = GW_ALV-ICDMB / GW_ALV-ICMNG.
          ENDIF.
        WHEN '23'.
          GW_ALV-IGMNG = GW_SUM_DATA-MENGE.
          GW_ALV-IGDMB = GW_SUM_DATA-DMBTR.
          IF GW_ALV-IGMNG IS NOT INITIAL.
            GW_ALV-IGAVP = GW_ALV-IGDMB / GW_ALV-IGMNG.
          ENDIF.
        WHEN '54'.
          GW_ALV-IJMNG = GW_SUM_DATA-MENGE.
          GW_ALV-IJDMB = GW_SUM_DATA-DMBTR.
          IF GW_ALV-IJMNG IS NOT INITIAL.
            GW_ALV-IJAVP = GW_ALV-IJDMB / GW_ALV-IJMNG.
          ENDIF.
        WHEN '41'.
          GW_ALV-IRMNG = GW_SUM_DATA-MENGE.
          GW_ALV-IRDMB = GW_SUM_DATA-DMBTR.
          IF GW_ALV-IRMNG IS NOT INITIAL.
            GW_ALV-IRAVP = GW_ALV-IRDMB / GW_ALV-IRMNG.
          ENDIF.
        WHEN '33'.
          GW_ALV-ILMNG = GW_SUM_DATA-MENGE.
          GW_ALV-ILDMB = GW_SUM_DATA-DMBTR.
          IF GW_ALV-ILMNG IS NOT INITIAL.
            GW_ALV-ILAVP = GW_ALV-ILDMB / GW_ALV-ILMNG.
          ENDIF.
        WHEN '61'.
          GW_ALV-IBMNG = GW_SUM_DATA-MENGE.
          GW_ALV-IBDMB = GW_SUM_DATA-DMBTR.
          IF GW_ALV-IBMNG IS NOT INITIAL.
            GW_ALV-IBAVP = GW_ALV-IBDMB / GW_ALV-IBMNG.
          ENDIF.
        WHEN '31'.
          GW_ALV-ORMNG = GW_SUM_DATA-MENGE.
          GW_ALV-ORDMB = GW_SUM_DATA-DMBTR.
          IF GW_ALV-ORMNG IS NOT INITIAL.
            GW_ALV-ORAVP = GW_ALV-ORDMB / GW_ALV-ORMNG.
          ENDIF.
        WHEN '32'.
          GW_ALV-OYMNG = GW_SUM_DATA-MENGE.
          GW_ALV-OYDMB = GW_SUM_DATA-DMBTR.
          IF GW_ALV-OYMNG IS NOT INITIAL.
            GW_ALV-OYAVP = GW_ALV-OYDMB / GW_ALV-OYMNG.
          ENDIF.
        WHEN '51'.
          GW_ALV-OXMNG = GW_SUM_DATA-MENGE.
          GW_ALV-OXDMB = GW_SUM_DATA-DMBTR.
          IF GW_ALV-OXMNG IS NOT INITIAL.
            GW_ALV-OXAVP = GW_ALV-OXDMB / GW_ALV-OXMNG.
          ENDIF.
        WHEN '53'.
          GW_ALV-OWMNG = GW_SUM_DATA-MENGE.
          GW_ALV-OWDMB = GW_SUM_DATA-DMBTR.
          IF GW_ALV-OWMNG IS NOT INITIAL.
            GW_ALV-OWAVP = GW_ALV-OWDMB / GW_ALV-OWMNG.
          ENDIF.
        WHEN '63'.
          GW_ALV-OPMNG = GW_SUM_DATA-MENGE.
          GW_ALV-OPDMB = GW_SUM_DATA-DMBTR.
          IF GW_ALV-OPMNG IS NOT INITIAL.
            GW_ALV-OPAVP = GW_ALV-OPDMB / GW_ALV-OPMNG.
          ENDIF.
        WHEN '24'.
          GW_ALV-OFMNG = GW_SUM_DATA-MENGE.
          GW_ALV-OFDMB = GW_SUM_DATA-DMBTR.
          IF GW_ALV-OFMNG IS NOT INITIAL.
            GW_ALV-OFAVP = GW_ALV-OFDMB / GW_ALV-OFMNG.
          ENDIF.
        WHEN '22'.
          GW_ALV-OGMNG = GW_SUM_DATA-MENGE.
          GW_ALV-OGDMB = GW_SUM_DATA-DMBTR.
          IF GW_ALV-OGMNG IS NOT INITIAL.
            GW_ALV-OGAVP = GW_ALV-OGDMB / GW_ALV-OGMNG.
          ENDIF.
        WHEN '55'.
          GW_ALV-OJMNG = GW_SUM_DATA-MENGE.
          GW_ALV-OJDMB = GW_SUM_DATA-DMBTR.
          IF GW_ALV-OJMNG IS NOT INITIAL.
            GW_ALV-OJAVP = GW_ALV-OJDMB / GW_ALV-OJMNG.
          ENDIF.
        WHEN '34'.
          GW_ALV-OLMNG = GW_SUM_DATA-MENGE.
          GW_ALV-OLDMB = GW_SUM_DATA-DMBTR.
          IF GW_ALV-OLMNG IS NOT INITIAL.
            GW_ALV-OLAVP = GW_ALV-OLDMB / GW_ALV-OLMNG.
          ENDIF.
        WHEN '62'.
          GW_ALV-OBMNG = GW_SUM_DATA-MENGE.
          GW_ALV-OBDMB = GW_SUM_DATA-DMBTR.
          IF GW_ALV-OBMNG IS NOT INITIAL.
            GW_ALV-OBAVP = GW_ALV-OBDMB / GW_ALV-OBMNG.
          ENDIF.
        WHEN '20'.
          GW_ALV-ODMNG = GW_SUM_DATA-MENGE.
          GW_ALV-ODDMB = GW_SUM_DATA-DMBTR.
          IF GW_ALV-ODMNG IS NOT INITIAL.
            GW_ALV-ODAVP = GW_ALV-ODDMB / GW_ALV-ODMNG.
          ENDIF.
        WHEN OTHERS.
      ENDCASE.
      ...
      DELETE GT_SUM_DATA INDEX L_TABIX.      "删除该条物料移动数据
    ENDLOOP.
    "CHANGED BY HAND HM AT 2015/04/20 15:39**********************START
    "入库总数（实际）
...
*********************ADD BY HAND HM AT 2015/03/18 21:16******************START
    "期初/末金额
    READ TABLE GT_ZFI035K INTO GW_ZFI035K WITH KEY
    MATNR = GW_ALV-MATNR
    BUKRS = GW_ALV-BUKRS
    ...
    IF SY-SUBRC = 0.
      ...
      MATNR = GW_ALV-MATNR
      BWKEY = GW_ALV-WERKS
      BWTAR = GW_ALV-CHARG BINARY SEARCH.
      IF SY-SUBRC <> 0.
        IF GW_ZFI035K-SMENG IS NOT INITIAL.
          GW_ALV-BDMBT = GW_ALV-BMENG / GW_ZFI035K-SMENG * GW_ZFI035K-SDMBT.
        ENDIF.
        IF GW_ZFI035K-EMENG IS NOT INITIAL.
          GW_ALV-EDMBT = GW_ALV-EMENG / GW_ZFI035K-EMENG * GW_ZFI035K-EDMBT.
        ENDIF.
      ENDIF.
    ENDIF.
    ...
    "CHANGED BY HAND HM AT 2015/04/20 15:39**********************START
    "入库分摊借 = 入库（计算） - 入库（实际）
    GW_ALV-DMBTR_S = GW_ALV-EDMBT - GW_ALV-SODMB - GW_ALV-BDMBT - GW_ALV-SIDMB.
*      "入库总数(计算)
    GW_ALV-SIMNG = GW_ALV-EMENG - GW_ALV-SOMNG - GW_ALV-BMENG.
    "入库总金额（计算）
    GW_ALV-SIDMB = GW_ALV-EDMBT - GW_ALV-SODMB - GW_ALV-BDMBT.

    IF GW_ALV-SIMNG IS NOT INITIAL.
      GW_ALV-SIAVP = GW_ALV-SIDMB / GW_ALV-SIMNG.
    ENDIF.

    IF GW_ALV-SOMNG IS NOT INITIAL.
      GW_ALV-SOAVP = GW_ALV-SODMB / GW_ALV-SOMNG.
    ENDIF.
    "*************************************************************END
...
**************************************************************************END
    ...
    "汇总期初库存,期末库存，出/入库数量,差异分摊借，用于后续分摊差异
    CLEAR GW_SUM_MBLNR.
*    GW_SUM_MBLNR-EDDAT = GW_ZFI035C-EDDAT.
*    GW_SUM_MBLNR-STDAT = GW_ZFI035C-STDAT.
    GW_SUM_MBLNR-MATNR = GW_ALV-MATNR.
    GW_SUM_MBLNR-WERKS = GW_ALV-WERKS.
    GW_SUM_MBLNR-SIMNG = GW_ALV-SIMNG."期间入库
    GW_SUM_MBLNR-SOMNG = GW_ALV-SOMNG."期间出库
    GW_SUM_MBLNR-BMENG = GW_ALV-BMENG.
    GW_SUM_MBLNR-BDMBT = GW_ALV-BDMBT.
    GW_SUM_MBLNR-EMENG = GW_ALV-EMENG.
    GW_SUM_MBLNR-EDMBT = GW_ALV-EDMBT.
    GW_SUM_MBLNR-DMBTR_S = GW_ALV-DMBTR_S.
    COLLECT GW_SUM_MBLNR INTO GT_SUM_MBLNR.
    "*****************************************************************END
*    "汇总期间入库总量，期初库存数量和期末库存总量
*    CLEAR GW_ALV_SUM.
*    GW_ALV_SUM-MATNR = GW_ALV-MATNR.
*    GW_ALV_SUM-BUKRS = GW_ALV-BUKRS.
*    GW_ALV_SUM-WERKS = GW_ALV-WERKS.
**    GW_ALV_SUM-LGORT = GW_ALV-LGORT.
**    GW_ALV_SUM-CHARG = GW_ALV-CHARG.
*    GW_ALV_SUM-SIMNG = GW_ALV-SIMNG.
*    GW_ALV_SUM-SIDMB = GW_ALV-SIDMB.
*    GW_ALV_SUM-BMENG = GW_ALV-BMENG.
*    GW_ALV_SUM-BDMBT = GW_ALV-BDMBT.
*    GW_ALV_SUM-EMENG = GW_ALV-EMENG.
*    GW_ALV_SUM-EDMBT = GW_ALV-EDMBT.
*    COLLECT GW_ALV_SUM INTO GT_ALV_SUM.
    "****************************************************************END
    APPEND GW_ALV TO GT_ALV.
    CLEAR GW_ALV.
  ENDLOOP.
*  CLEAR GT_SUM_MBLNR.
  "若还有物料移动数据，则对应的物料期初库存为0，因此未加入到ALV中。
  "根据移动组对应的数量和金额放到相应ALV字段***********************START
  SORT GT_SUM_DATA BY MATNR MAKTX BUKRS WERKS LGORT CHARG.
  LOOP AT GT_SUM_DATA INTO GW_SUM_DATA.

    LOOP AT GT_SUM_MSEG INTO GW_SUM_MSEG
      WHERE MATNR = GW_SUM_DATA-MATNR
        AND BUKRS = GW_SUM_DATA-BUKRS
        AND WERKS = GW_SUM_DATA-WERKS
        AND LGORT = GW_SUM_DATA-LGORT
        AND CHARG = GW_SUM_DATA-CHARG.
      L_TABIX_X = SY-TABIX.
      IF GW_SUM_MSEG-MVGRP = '11' OR GW_SUM_MSEG-MVGRP = '21' OR GW_SUM_MSEG-MVGRP = '52'    "HANDWY
      OR GW_SUM_MSEG-MVGRP = '62' OR GW_SUM_MSEG-MVGRP = '42' OR GW_SUM_MSEG-MVGRP = '43'
      OR GW_SUM_MSEG-MVGRP = '54' OR GW_SUM_MSEG-MVGRP = '23' OR GW_SUM_MSEG-MVGRP = '41'
    ...
    AT END OF CHARG.
      ...
      "期末库存
      READ TABLE GT_ZFI035C INTO GW_ZFI035C WITH KEY
      MATNR = GW_ALV-MATNR
      BUKRS = GW_ALV-BUKRS
      WERKS = GW_ALV-WERKS
      LGORT = GW_ALV-LGORT
...
*******************************************************************END
  "分摊差异贷*****************************************************START
  MOVE-CORRESPONDING GT_SUM_MBLNR TO GT_SUM_DMBTR.
  SORT GT_SUM_DMBTR BY MATNR WERKS STDAT EDDAT.
  SORT GT_SUM_BELNR BY MATNR WERKS.
  SORT GT_SUM_MBLNR BY MATNR WERKS.
  LOOP AT GT_ALV INTO GW_ALV.
    "处理期初库存/金额的差异（尾差）
    LOOP AT GT_SUM_DMBTR INTO GW_SUM_DMBTR
    WHERE MATNR = GW_ALV-MATNR
      ...
      MATNR = GW_ALV-MATNR
      WERKS = GW_ALV-WERKS BINARY SEARCH.
      IF SY-SUBRC = 0 AND GW_ALV-BMENG IS NOT INITIAL AND GW_ALV-BDMBT IS NOT INITIAL.
        GW_SUM_DMBTR-CONTA = '1'.
*        GW_ALV-EMENG = GW_ALV-EMENG + GW_ZFI035K-EMENG - GW_SUM_DMBTR-EMENG.
*        GW_ALV-EDMBT = GW_ALV-EDMBT + GW_ZFI035K-EDMBT - GW_SUM_DMBTR-EDMBT.
        GW_ALV-BMENG = GW_ALV-BMENG + GW_ZFI035K-SMENG - GW_SUM_DMBTR-BMENG.
        GW_ALV-BDMBT = GW_ALV-BDMBT + GW_ZFI035K-SDMBT - GW_SUM_DMBTR-BDMBT.
      ENDIF.
*      DELETE GT_SUM_DMBTR INDEX L_TABIX.
      MODIFY GT_SUM_DMBTR FROM GW_SUM_DMBTR.
    ENDLOOP.
    ...
    WHERE MATNR = GW_ALV-MATNR
      ...
      MATNR = GW_ALV-MATNR
      WERKS = GW_ALV-WERKS BINARY SEARCH.
      IF SY-SUBRC = 0 AND GW_ALV-EMENG IS NOT INITIAL AND GW_ALV-EDMBT IS NOT INITIAL.
        GW_SUM_DMBTR-CONTB = '1'.
        GW_ALV-EMENG = GW_ALV-EMENG + GW_ZFI035K-EMENG - GW_SUM_DMBTR-EMENG.
        GW_ALV-EDMBT = GW_ALV-EDMBT + GW_ZFI035K-EDMBT - GW_SUM_DMBTR-EDMBT.
*        GW_ALV-BMENG = GW_ALV-BMENG + GW_ZFI035K-SMENG - GW_SUM_DMBTR-BMENG.
*        GW_ALV-BDMBT = GW_ALV-BDMBT + GW_ZFI035K-SDMBT - GW_SUM_DMBTR-BDMBT.
...
*      BUKRS = GW_ALV-BUKRS
      WERKS = GW_ALV-WERKS BINARY SEARCH.
      IF SY-SUBRC = 0 AND GW_SUM_MBLNR-EMENG IS NOT INITIAL AND GW_ALV-EMENG IS NOT INITIAL.
        GW_ALV-DMBTR_H = GW_SUM_BELNR-DMBTR / GW_SUM_MBLNR-EMENG * GW_ALV-SOMNG.
*        GW_ALV-DMBTR_H =  GW_SUM_BELNR-DMBTR / GW_SUM_MBLNR-EDMBT * GW_ALV-SODMB * -1.
      ENDIF.
    ENDIF.
****************CHANGED BY HAND HM AT 20150512 19:05*******************************START
*    READ TABLE GT_SUM_MBLNR INTO GW_SUM_MBLNR WITH KEY
*    MATNR = GW_ALV-MATNR
*    WERKS = GW_ALV-WERKS BINARY SEARCH.
*    IF SY-SUBRC = 0 AND GW_SUM_MBLNR-SIMNG IS NOT INITIAL.
*      GW_ALV-DMBTR_S = GW_ALV-SIMNG / GW_SUM_MBLNR-SIMNG * GW_SUM_MBLNR-DMBTR_S.
*    ENDIF.
************************************************************************************END
    "对分摊的差异借/贷求和，因为分摊会产生尾差，用求和的金额和分摊金额得到尾差，
    "再对尾差进行处理
    GW_ALV_TEMP-MATNR = GW_ALV-MATNR.
    GW_ALV_TEMP-WERKS = GW_ALV-WERKS.
    GW_ALV_TEMP-BDMBT = GW_ALV-BDMBT.
*    GW_ALV_TEMP-EDMBT = GW_ALV-EDMBT.
...
*      "入库总数(计算)
  ...
  "处理尾差，加到第一行上去
****************CHANGED BY HAND HM AT 20150512 19:05***********************START
*  SORT GT_ALV_TEMP BY MATNR WERKS.
*  LOOP AT GT_ALV INTO GW_ALV.
*    CLEAR: L_S,L_H,L_B.
*    READ TABLE GT_ALV_TEMP INTO GW_ALV_TEMP WITH KEY
*    MATNR = GW_ALV-MATNR
*    WERKS = GW_ALV-WERKS BINARY SEARCH.
*    IF SY-SUBRC = 0.
*      L_TABIX = SY-TABIX.
**      L_S = GW_ALV_TEMP-DMBTR_S.
************************CHANGED BY HAND HM AT 20150512 10:11********************START
**      L_H = GW_ALV_TEMP-DMBTR_H.
**     "差异分摊贷尾差
**      READ TABLE GT_SUM_BELNR INTO GW_SUM_BELNR WITH KEY
**      MATNR = GW_ALV-MATNR
**      WERKS = GW_ALV-WERKS BINARY SEARCH.
...
**      WERKS = GW_ALV-WERKS BINARY SEARCH.
**      IF SY-SUBRC = 0.
**        L_S = GW_ALV_TEMP-DMBTR_S - GW_SUM_MBLNR-DMBTR_S.
**      ENDIF.
********************************************************************************END
**      READ TABLE GT_ZFI035K INTO GW_ZFI035K WITH KEY
**      MATNR = GW_ALV-MATNR
**      WERKS = GW_ALV-WERKS BINARY SEARCH.
**      IF SY-SUBRC = 0.
**        L_B = GW_ALV_TEMP-BDMBT - GW_ZFI035K-SDMBT.
**      ENDIF.
*
...
FORM FRM_SET_FIELDCAT .
  ...
  IF P_GZ IS INITIAL AND P_WJ IS INITIAL AND P_BH IS INITIAL AND P_PC IS INITIAL.
    ...
    SET_FIELDCAT GW_FIELDCAT GT_FIELDCAT 'MATKL' '物料组'.
    SET_FIELDCAT GW_FIELDCAT GT_FIELDCAT 'MVGRP' '移动组'.
  ENDIF.
  SET_FIELDCAT GW_FIELDCAT GT_FIELDCAT 'CHARG' '批次'.
  SET_FIELDCAT GW_FIELDCAT GT_FIELDCAT 'BKLAS' '评估类'.
  ...
  SET_FIELDCAT GW_FIELDCAT GT_FIELDCAT 'SIMNG_X' '入库总数量(排除金额为0部分)'.
  SET_FIELDCAT GW_FIELDCAT GT_FIELDCAT 'SIAVP' '入库平均单位成本'.
  SET_FIELDCAT GW_FIELDCAT GT_FIELDCAT 'SIDMB' '入库总金额'.
  ...
  SET_FIELDCAT GW_FIELDCAT GT_FIELDCAT 'SOMNG_X' '出库总数量(排除金额为0部分)'.
  SET_FIELDCAT GW_FIELDCAT GT_FIELDCAT 'SOAVP' '出库平均单位成本'.
  SET_FIELDCAT GW_FIELDCAT GT_FIELDCAT 'SODMB' '出库总金额'.
...
*    GW_SUBALV-EDMBT = GW_ZFI035P-EDMBT.
    CASE GW_ZFI035P-MVGRP.
      WHEN '11'.
        ...
        IF GW_SUBALV-IMMNG IS NOT INITIAL.
          GW_SUBALV-IMAVP = GW_SUBALV-IMDMB / GW_SUBALV-IMMNG.
        ENDIF.
      WHEN '21'.
        GW_SUBALV-IDMNG = GW_ZFI035P-MENGE.
        GW_SUBALV-IDDMB = GW_ZFI035P-DMBTR.
        IF GW_SUBALV-IDMNG IS NOT INITIAL.
          GW_SUBALV-IDAVP = GW_SUBALV-IDDMB / GW_SUBALV-IDMNG.
        ENDIF.
      WHEN '52'.
        GW_SUBALV-IWMNG = GW_ZFI035P-MENGE.
        GW_SUBALV-IWDMB = GW_ZFI035P-DMBTR.
        IF GW_SUBALV-IWMNG IS NOT INITIAL.
          GW_SUBALV-IWAVP = GW_SUBALV-IWDMB / GW_SUBALV-IWMNG.
        ENDIF.
      WHEN '62'.
        GW_SUBALV-IPMNG = GW_ZFI035P-MENGE.
        GW_SUBALV-IPDMB = GW_ZFI035P-DMBTR.
        IF GW_SUBALV-IPMNG IS NOT INITIAL.
          GW_SUBALV-IPAVP = GW_SUBALV-IPDMB / GW_SUBALV-IPMNG.
        ENDIF.
      WHEN '42'.
        GW_SUBALV-IHMNG = GW_ZFI035P-MENGE.
        GW_SUBALV-IHDMB = GW_ZFI035P-DMBTR.
        IF GW_SUBALV-IHMNG IS NOT INITIAL.
          GW_SUBALV-IHAVP = GW_SUBALV-IHDMB / GW_SUBALV-IHMNG.
        ENDIF.
      WHEN '43'.
        GW_SUBALV-ICMNG = GW_ZFI035P-MENGE.
        GW_SUBALV-ICDMB = GW_ZFI035P-DMBTR.
        IF GW_SUBALV-ICMNG IS NOT INITIAL.
          GW_SUBALV-ICAVP = GW_SUBALV-ICDMB / GW_SUBALV-ICMNG.
        ENDIF.
      WHEN '23'.
        GW_SUBALV-IGMNG = GW_ZFI035P-MENGE.
        GW_SUBALV-IGDMB = GW_ZFI035P-DMBTR.
        IF GW_SUBALV-IGMNG IS NOT INITIAL.
          GW_SUBALV-IGAVP = GW_SUBALV-IGDMB / GW_SUBALV-IGMNG.
        ENDIF.
      WHEN '54'.
        GW_SUBALV-IJMNG = GW_ZFI035P-MENGE.
        GW_SUBALV-IJDMB = GW_ZFI035P-DMBTR.
        IF GW_SUBALV-IJMNG IS NOT INITIAL.
          GW_SUBALV-IJAVP = GW_SUBALV-IJDMB / GW_SUBALV-IJMNG.
        ENDIF.
      WHEN '41'.
        GW_SUBALV-IRMNG = GW_ZFI035P-MENGE.
        GW_SUBALV-IRDMB = GW_ZFI035P-DMBTR.
        IF GW_SUBALV-IRMNG IS NOT INITIAL.
          GW_SUBALV-IRAVP = GW_SUBALV-IRDMB / GW_SUBALV-IRMNG.
        ENDIF.
      WHEN '33'.
        GW_SUBALV-ILMNG = GW_ZFI035P-MENGE.
        GW_SUBALV-ILDMB = GW_ZFI035P-DMBTR.
        IF GW_SUBALV-ILMNG IS NOT INITIAL.
          GW_SUBALV-ILAVP = GW_SUBALV-ILDMB / GW_SUBALV-ILMNG.
        ENDIF.
      WHEN '61'.
        GW_SUBALV-IBMNG = GW_ZFI035P-MENGE.
        GW_SUBALV-IBDMB = GW_ZFI035P-DMBTR.
        IF GW_SUBALV-IBMNG IS NOT INITIAL.
          GW_SUBALV-IBAVP = GW_SUBALV-IBDMB / GW_SUBALV-IBMNG.
        ENDIF.
      WHEN '31'.
        GW_SUBALV-ORMNG = GW_ZFI035P-MENGE.
        GW_SUBALV-ORDMB = GW_ZFI035P-DMBTR.
        IF GW_SUBALV-ORMNG IS NOT INITIAL.
          GW_SUBALV-ORAVP = GW_SUBALV-ORDMB / GW_SUBALV-ORMNG.
        ENDIF.
      WHEN '32'.
        GW_SUBALV-OYMNG = GW_ZFI035P-MENGE.
        GW_SUBALV-OYDMB = GW_ZFI035P-DMBTR.
        IF GW_SUBALV-OYMNG IS NOT INITIAL.
          GW_SUBALV-OYAVP = GW_SUBALV-OYDMB / GW_SUBALV-OYMNG.
        ENDIF.
      WHEN '51'.
        GW_SUBALV-OXMNG = GW_ZFI035P-MENGE.
        GW_SUBALV-OXDMB = GW_ZFI035P-DMBTR.
        IF GW_SUBALV-OXMNG IS NOT INITIAL.
          GW_SUBALV-OXAVP = GW_SUBALV-OXDMB / GW_SUBALV-OXMNG.
        ENDIF.
      WHEN '53'.
        GW_SUBALV-OWMNG = GW_ZFI035P-MENGE.
        GW_SUBALV-OWDMB = GW_ZFI035P-DMBTR.
        IF GW_SUBALV-OWMNG IS NOT INITIAL.
          GW_SUBALV-OWAVP = GW_SUBALV-OWDMB / GW_SUBALV-OWMNG.
        ENDIF.
      WHEN '63'.
        GW_SUBALV-OPMNG = GW_ZFI035P-MENGE.
        GW_SUBALV-OPDMB = GW_ZFI035P-DMBTR.
        IF GW_SUBALV-OPMNG IS NOT INITIAL.
          GW_SUBALV-OPAVP = GW_SUBALV-OPDMB / GW_SUBALV-OPMNG.
        ENDIF.
      WHEN '24'.
        GW_SUBALV-OFMNG = GW_ZFI035P-MENGE.
        GW_SUBALV-OFDMB = GW_ZFI035P-DMBTR.
        IF GW_SUBALV-OFMNG IS NOT INITIAL.
          GW_SUBALV-OFAVP = GW_SUBALV-OFDMB / GW_SUBALV-OFMNG.
        ENDIF.
      WHEN '22'.
        GW_SUBALV-OGMNG = GW_ZFI035P-MENGE.
        GW_SUBALV-OGDMB = GW_ZFI035P-DMBTR.
        IF GW_SUBALV-OGMNG IS NOT INITIAL.
          GW_SUBALV-OGAVP = GW_SUBALV-OGDMB / GW_SUBALV-OGMNG.
        ENDIF.
      WHEN '55'.
        GW_SUBALV-OJMNG = GW_ZFI035P-MENGE.
        GW_SUBALV-OJDMB = GW_ZFI035P-DMBTR.
        IF GW_SUBALV-OJMNG IS NOT INITIAL.
          GW_SUBALV-OJAVP = GW_SUBALV-OJDMB / GW_SUBALV-OJMNG.
        ENDIF.
      WHEN '34'.
        GW_SUBALV-OLMNG = GW_ZFI035P-MENGE.
        GW_SUBALV-OLDMB = GW_ZFI035P-DMBTR.
        IF GW_SUBALV-OLMNG IS NOT INITIAL.
          GW_SUBALV-OLAVP = GW_SUBALV-OLDMB / GW_SUBALV-OLMNG.
        ENDIF.
      WHEN '62'.
        GW_SUBALV-OBMNG = GW_ZFI035P-MENGE.
        GW_SUBALV-OBDMB = GW_ZFI035P-DMBTR.
        IF GW_SUBALV-OBMNG IS NOT INITIAL.
          GW_SUBALV-OBAVP = GW_SUBALV-OBDMB / GW_SUBALV-OBMNG.
        ENDIF.
      WHEN '20'.
        GW_SUBALV-ODMNG = GW_ZFI035P-MENGE.
        GW_SUBALV-ODDMB = GW_ZFI035P-DMBTR.
        IF GW_SUBALV-ODMNG IS NOT INITIAL.
          GW_SUBALV-ODAVP = GW_SUBALV-ODDMB / GW_SUBALV-ODMNG.
        ENDIF.
      WHEN OTHERS.
    ENDCASE.
    AT END OF SOBKZ.
...
*      GW_SUBALV-MVGRP = GW_ZFI035P-MVGRP.
      ...
      IF SY-SUBRC = 0.
        ...
        IF GW_ZFI035K-EMENG IS NOT INITIAL.
          ...
          IF GW_SUBALV-EMENG IS NOT INITIAL.
...
*----------------------------------------------------------------------*
*FORM FRM_GET_TIMES .
*  DATA: L_ZINDEX TYPE ZFI035T-ZINDEX.
*  SELECT MAX( ZINDEX )
*    FROM ZFI035T
*    INTO L_ZINDEX.
*  L_ZINDEX = L_ZINDEX + 1.
*  LW_ZFI035T-ZINDEX = L_ZINDEX.
*ENDFORM.                    " FRM_GET_TIMES
*&---------------------------------------------------------------------*
*&      Form  FRM_JOB
*&---------------------------------------------------------------------*
*       text
...
FORM FRM_JOB .
  ...
  GET TIME.
  TIM = SY-UZEIT + 20.              "系统当期时间后20秒
  CONCATENATE 'ZFIR104' SY-DATUM SY-UZEIT INTO L_STRING.
  GLOBAL_JOB-JOBNAME = L_STRING. "作业名
...
* Implement suitable error handling here
  ...
  LOOP AT I_VALUTAB.
    CASE I_VALUTAB-SELNAME.
      WHEN 'P_BUKRS'.
        I_VALUTAB-LOW = P_BUKRS.
        CLEAR I_VALUTAB-SIGN.
        CLEAR I_VALUTAB-OPTION.
      WHEN 'S_WERKS'.
        I_VALUTAB-LOW = S_WERKS-LOW.
        I_VALUTAB-HIGH = S_WERKS-HIGH.
        I_VALUTAB-SIGN = S_WERKS-SIGN.
        I_VALUTAB-OPTION = S_WERKS-OPTION.
      WHEN 'P_STDAT'.
        I_VALUTAB-LOW = P_STDAT.
        CLEAR I_VALUTAB-SIGN.
        CLEAR I_VALUTAB-OPTION.
      WHEN 'P_EDDAT'.
        I_VALUTAB-LOW = P_EDDAT.
        CLEAR I_VALUTAB-SIGN.
        CLEAR I_VALUTAB-OPTION.
    ENDCASE.
    MODIFY I_VALUTAB.
...
*----------------------------------------------------------------------*
FORM FRM_CHECK_PLANT .
  DATA: ITAB_001W TYPE STANDARD TABLE OF T001W WITH HEADER LINE.
...
* Implement suitable error handling here
  ENDIF.
  SORT ITAB_001W BY WERKS ASCENDING.
  L_WERKS = S_WERKS-LOW.
  IF S_WERKS-HIGH = ''.
    S_WERKS-HIGH = S_WERKS-LOW.
  ENDIF.
  WHILE L_WERKS <= S_WERKS-HIGH.
    READ TABLE ITAB_001W WITH KEY WERKS+0(2) = L_WERKS+0(2).
    IF SY-SUBRC <> 0.
      CLEAR S_WERKS-HIGH.
      MESSAGE '该工厂范围不在公司代码内' TYPE 'E'.
    ENDIF.
    L_WERKS = L_WERKS + 1.
  ENDWHILE.

...
*判断图片编号是否等于18位
  LOOP AT FILETAB.
    ...
    IF LEN <> 18 .
      FLAG = 'X'.
      CONCATENATE '@5C@' '文件' FILETAB-FILENAME  '的文件名' FILENAME '长度必须等于18位' INTO MESS.
      CONDENSE MESS.
      WRITE: / MESS.
    ENDIF.
  ENDLOOP.
...
*判断员工编号是否存在
    SELECT SINGLE *
     ...
     WHERE SUBTY = '01'
       ...
       AND ICNUM = FILETAB-FILENAME+0(18).
    IF SY-SUBRC = 0.

      CONCATENATE PA0185-PERNR '0002' INTO SAPOBJID.
      ...
      ELSE.
        WRITE : / '@5B@', 'Upload ',SAPPFAD,'身份证号 ',FILETAB-FILENAME+0(18),'员工号',PA0185-PERNR,'Sccuess!'.
      ENDIF.

    ELSE.
      WRITE : / '@5C@', 'ERROR  ',SAPPFAD, '此员工身份证号 ', FILETAB-FILENAME+0(18),'不存在!'.
    ENDIF.

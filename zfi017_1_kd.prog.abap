...
*& --------------------------13074  -------insert 0807 -----------------
DATA GT_DATA_KD TYPE TABLE OF TY_DATA.
*& ----------------------------------------------------------------------
"HANDLH ADD
...
* -------------------------- 13074 ----------0803-----
DATA : GT_ZFI TYPE TABLE OF ZFI017_KD,
       GS_ZFI LIKE LINE OF GT_ZFI.
* ----------------------------------------------------
...
*ADD BY HANDWY  210-7-16 增加二分法排序
  ...
  SORT GT_T052U1  BY  ZTERM.   " NEW INSERT 13074
  SORT GT_T052S   BY  ZTERM.   " NEW  INSERT 13074
*ENDADD.
...
**        GS_DATA-ZTERM     = GS_EKKO-ZTERM.   " 13074
*
**子付款条件描述
*
**   -----------------------13074 ------zhushi ----
...
**  --------------------------13074 ------INSERT ------------
**   订单超额金额
*      IF GS_DATA-YFJE > GS_DATA-YFJE1.
*                GS_DATA-ZCEJE = GS_DATA-YFJE - GS_DATA-YFJE1.
*      ENDIF.
*
*
** ----------------------------------------------------------
...
*        ENDIF.
**        APPEND GS_DATA TO GT_DATA.    " 13074  ZHUSHI
**        CLEAR GS_DATA.                " 13074  ZHUSHI
*      ELSE.
*        CLEAR L_TABIX.
...
**          GS_DATA-ZTERM = GS_T052S-RATZT.    " 13074
*
*          L_TABIX = L_TABIX + 1.
*          GS_DATA-ZCH   = L_TABIX.
*
...
*  应付金额 & 应付比例
    ...
    IF SY-SUBRC = 0.
      ...
      ELSE.
        READ TABLE GT_T052S INTO GS_T052S
                        WITH KEY ZTERM = GS_EKKO-ZTERM BINARY SEARCH.
          GS_DATA-YFJE1 = GS_DATA-NETWR_1." * GS_T052S-RATPZ / 100.
          GS_DATA-RATPZ = GS_T052S-RATPZ.
...
*已付比例
     IF GS_DATA-YFJE1 NE  0.
        GS_DATA-YFBL  = ( GS_DATA-YFJE / GS_DATA-NETWR_1 ) * 100.
     ENDIF.
*        APPEND GS_DATA TO GT_DATA.    " 13074  ZHUSHI
*        CLEAR GS_DATA.                " 13074  ZHUSHI
...
*  --------------------------13074 ------INSERT ------------
*   订单超额金额
     GS_DATA-ZCEJE = GS_DATA-YFJE - GS_DATA-NETWR_1.
*   订单可申请金额
     GS_DATA-ZKSQJE = GS_DATA-NETWR_1 - GS_DATA-YSQJE.
...
*   -----------------13074 insert -----------
     READ TABLE GT_T052U1   INTO GS_T052U1
        WITH KEY ZTERM = GS_DATA-ZTERM BINARY SEARCH.
        IF SY-SUBRC = 0.
          GS_DATA-TEXT1 = GS_T052U1-TEXT1.
        ENDIF.
...
*------------------------------------13074 isnert ---------------------------
    READ TABLE LT_EKKO INTO LS_EKKO WITH KEY EBELN = GS_DATA-EBELN BINARY SEARCH .
      IF SY-SUBRC = 0 .
        ...
        CLEAR : LS_DATA2-YJFKJE , LS_DATA2-YJFKRQ.
...
*节点已付金额
    ...
    CLEAR : GS_DATA-ZJDYSQJE , GS_DATA-ZJDYFJE.
    IF GS_DATA-ZJDFKJH = '1'.
      LOOP AT  GT_ZFI017 INTO GS_ZFI017
      WHERE EBELN = GS_DATA-EBELN
      AND   ZCLJD = '3'
      AND   ZJDFKJH = '1'.
        GS_DATA-ZJDYFJE = GS_DATA-ZJDYFJE + GS_ZFI017-ZSQFKJE.
      ENDLOOP.
*节点已申请金额
      ...
      LOOP AT GT_ZFI017 INTO GS_ZFI017
       ...
       OR   ZCLJD  = '1')
       AND   STATU    <> ICON_DELETE.
        GS_DATA-ZJDYSQJE = GS_DATA-ZJDYSQJE + GS_ZFI017-ZSQFKJE.
      ENDLOOP.
    ELSEIF GS_DATA-ZJDFKJH = '2'.
...
*节点已申请金额
  ...
  CLEAR : GS_ZFI.
  LOOP AT GT_DATA INTO GS_DATA.
    IF ( GS_DATA-EBELN NE GS_ZFI-EBELN ) AND ( GS_DATA-ZJDFKJH NE GS_ZFI-ZJDFKJH ).
      MOVE-CORRESPONDING GS_DATA TO GS_ZFI.
      INSERT  ZFI017_KD  FROM GS_ZFI.
      CLEAR GS_ZFI.
    ENDIF.
  ENDLOOP.
...
FORM FRM_PRINT .
  ...
  DATA:L_FORMNAME TYPE TDSFNAME VALUE 'ZSFFI017_1_KD'.
...
*&--------------------------ADD BY HANDZFF 20170516 15:34 ---------------------------*
  IF S_BUKRS-LOW = '1700' OR S_BUKRS-LOW = '1710'." OR S_BUKRS-LOW = '1720'. by 13074 kd 20170818
    L_FORMNAME = 'ZSFFI017_1_1_KD'.
  ENDIF.
*&--------------------------END BY HANDZFF 20170516 15:34 ---------------------------*
  IF S_BUKRS-LOW = '1720'.
    L_FORMNAME = 'ZSFFI017_KD_1720'.
  ENDIF.
*查询打印的次数
...
*-----------保修期限和保修起始日期----------------
      ...
      L_BZ = GS_DATA-WAERS.    " BY HANDKD 13074  0820
    ENDLOOP.
    LS_DATA1 = GS_DATA.

...
*&--代码添加 BY HANDYBY 23.06.2017 17:58:30  BEGIN
       KTGRM       TYPE TVKMT-KTGRM, "科目设置组
       VTEXT       TYPE TVKMT-VTEXT, "科目设置组描述
*&--代码添加 BY HANDYBY 23.06.2017 17:58:30  END
       SEL(1),
     END OF GS_DATA.
...
FORM FRM_GET_DATA .
  SELECT A~BUKRS A~GJAHR A~BELNR A~BLART A~BUDAT A~WAERS AS WAERS_1 A~BKTXT A~XBLNR A~AWTYP
       B~BUZEI B~SHKZG B~DMBTR B~WRBTR B~XNEGP B~HKONT B~ZUONR B~EBELN B~MATNR B~VBEL2 B~VBUND
  ...
  IF GT_DATA[] IS NOT INITIAL.
    ...
    SELECT * INTO CORRESPONDING FIELDS OF TABLE GT_SKAT FROM SKAT WHERE KTOPL = '1000' AND SAKNR BETWEEN '6401000000' AND '6401999999'.
...
*&--代码添加 BY HANDYBY 21.06.2017 09:58:46  BEGIN
*  DATA RG_YWLX TYPE RANGE OF STRING WITH HEADER LINE .
**  RANGES RG_YWLX2 FOR STRING .
*  RG_YWLX-SIGN = 'I'.
*  RG_YWLX-OPTION = 'CP'.
*  RG_YWLX-LOW = 'XC*'.
**  RG_YWLX-SIGN = 'I'.
*  APPEND RG_YWLX .
  ...
  DATA: LT_TVKMT TYPE TABLE OF TVKMT,
        LS_TVKMT TYPE TVKMT.
  SELECT SINGLE *
    INTO LS_TVKMT
    FROM TVKMT
   WHERE KTGRM = 'Z9'
     AND SPRAS = SY-LANGU .
*&--代码添加 BY HANDYBY 21.06.2017 09:58:46  END
  ...
  LOOP AT GT_DATA ASSIGNING <FS_DATA> .
    ...
    IF <FS_DATA>-BLART = 'ML'  .
      ...
      READ TABLE GT_VBAK WITH KEY VBELN = <FS_DATA>-ZUONR BINARY SEARCH.
      IF SY-SUBRC = 0 .

        IF  GT_VBAK-AUART = 'ZF2' OR GT_VBAK-AUART = 'ZOR' OR GT_VBAK-AUART = 'ZRE' OR  GT_VBAK-AUART = 'ZSD'.
          <FS_DATA>-YWLX = '产品'.
          "销售订单
          <FS_DATA>-VBELN = <FS_DATA>-ZUONR .
          "客户
          <FS_DATA>-KUNNR = GT_VBAK-KUNNR.
          "项目名称
          PERFORM SELXMMC USING <FS_DATA>-VBELN CHANGING <FS_DATA>-XMMC.
      ...
      ENDIF.
    ENDIF.
    IF <FS_DATA>-BLART = 'WL' AND <FS_DATA>-EBELN  NE ''.
      <FS_DATA>-YWLX = '公司间'.
      <FS_DATA>-VBELN = <FS_DATA>-EBELN.
    ...
    IF <FS_DATA>-BLART = 'ML' AND <FS_DATA>-AWTYP  EQ 'MLHD' AND <FS_DATA>-HKONT EQ '6401020101' .
      <FS_DATA>-YWLX = '公司间'.
      <FS_DATA>-VBELN = <FS_DATA>-EBELN.
      <FS_DATA>-KUNNR = <FS_DATA>-VBUND.
      "加上前导零函数
    ...
    IF <FS_DATA>-YWLX NE '项目' AND  <FS_DATA>-YWLX NE '产品' AND  <FS_DATA>-YWLX NE '公司间'.
      <FS_DATA>-YWLX = '其他'.
      <FS_DATA>-KUNNR = <FS_DATA>-VBUND.
      "加上前导零函数
      CALL FUNCTION 'CONVERSION_EXIT_ALPHA_INPUT'
...
*&--代码添加 BY HANDYBY 21.06.2017 10:26:14  BEGIN
  LOOP AT GT_DATA ASSIGNING <FS_DATA> .

    L_INDEX = SY-TABIX .
    READ TABLE LT_MVKE INTO LS_MVKE WITH KEY MATNR = <FS_DATA>-MATNR BINARY SEARCH .
    IF SY-SUBRC = 0 .
      IF LS_MVKE-KTGRM = 'Z9'.
        <FS_DATA>-KTGRM = LS_TVKMT-KTGRM .
        <FS_DATA>-VTEXT = LS_TVKMT-VTEXT .
      ENDIF.
    ENDIF.
    MODIFY GT_DATA FROM <FS_DATA> INDEX L_INDEX .
    CLEAR LS_MVKE .
    CLEAR L_INDEX .

  ENDLOOP.
*&--代码添加 BY HANDYBY 21.06.2017 10:26:14  END
...
*&--代码添加 BY HANDYBY 23.06.2017 17:57:53  BEGIN
  INIT_FIELDCAT 'KTGRM'          '科目设置组'         '' '' '' '' '' '' ''.
  INIT_FIELDCAT 'VTEXT'          '科目设置组描述'         '' '' '' '' '' '' ''.
*&--代码添加 BY HANDYBY 23.06.2017 17:57:53  END
ENDFORM.
*&---------------------------------------------------------------------*

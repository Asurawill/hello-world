...
DATA:gt_bkpf_999 type table of bkpf,
     gs_bkpf_999 type bkpf.
...
"查询会计凭证
 ...
 if gt_bkpf is not initial.
     select * into table gt_bkpf_999
       ...
       and   numpg eq '999'.
     sort gt_bkpf_999 by bukrs belnr gjahr .
...
*        pspid
    ...
    ENDIF.
     ...
     "替换：凭证编码字段
    read table gt_bkpf_999 into gs_bkpf_999 with key bukrs = gs_data-bukrs
                                                     belnr = gs_data-belnr
                                                     gjahr = gs_data-gjahr
                                             binary search.
         if sy-subrc eq 0.
             gs_data-belnr = gs_bkpf_999-xblnr.
             clear:gs_data-awkey.              "更换凭证编号
             CONCATENATE gs_bkpf_999-xblnr gs_bkpf_999-bukrs gs_bkpf_999-gjahr into gs_data-awkey.        "发票凭证号
         endif.
...
FORM frm_print_data .
  ...
  DATA:l_formname TYPE tdsfname VALUE 'ZSFFI002'.
...
FORM frm_print_data1 .
  ...
  DATA: emptycount      TYPE i VALUE 0,  "空行数.
        ...
        job_output_info TYPE ssfcrescl.
  DATA: g_name TYPE rs38l_fnam.
  DATA:l_formname TYPE tdsfname VALUE 'ZSFFI002_2'.
  DATA l_line TYPE i. "统计打印的行进行补行
  DATA g_line TYPE i. "设定换页行数
  DATA g_all_s TYPE dmbtr.
  DATA g_all_h TYPE dmbtr.

...
DATA:gt_sel TYPE TABLE OF ty_data,
     gs_sel TYPE ty_data.
...
PARAMETERS:P_X AS CHECKBOX  DEFAULT 'X'.
...
FORM frm_init_lvc .
  ...
  init_fieldcat 'VBELN'            '项目订单'               '' '' '' 'X' '' 'VBAK' 'VBELN'.
...
FORM frm_data_changed USING er_data_changed TYPE REF TO cl_alv_changed_data_protocol.
  ...
  DATA: ls_mod_cell   TYPE lvc_s_modi,
        ...
        lv_value_xmms TYPE lvc_value.

  DATA:p_xmmc  TYPE string,
       p_vbeln TYPE vbeln.
  ...
  LOOP AT  er_data_changed->mt_mod_cells
    ...
    IF sy-subrc = 0.
      "获取项目名称》
      CLEAR:p_xmmc.
      p_vbeln = ls_mod_cell-value .

      SELECT SINGLE * INTO gs_vbak
        FROM vbak
        WHERE vbeln = p_vbeln.
      IF sy-subrc EQ 0.
        PERFORM selxmmc USING p_vbeln sy-langu CHANGING p_xmmc.
        lv_value_xmms =  p_xmmc.
      ELSE.
      ...
      IF p_xmmc IS NOT INITIAL.
        CALL METHOD er_data_changed->modify_cell
          EXPORTING
            i_row_id    = ls_mod_cell-row_id
            i_fieldname = 'XMMS'
            i_value     = lv_value_xmms.
...
FORM frm_save_data .
  ...
  READ TABLE GT_SEL INTO GS_SEL WITH KEY VBELN = ''.
   IF SY-SUBRC EQ 0 .
     MESSAGE '要保存行项目记录的项目订单为空，不能保存！'  TYPE 'E'.
   ENDIF.
  SORT gt_sel BY mblnr mjahr zeile .
  LOOP AT gt_sel INTO gs_sel .
    CLEAR:gs_zzbksj .
    MOVE-CORRESPONDING gs_sel TO gs_zzbksj .

    "手工项目号
    gs_zzbksj-vbeln_sg = gs_sel-vbeln .

...
*&--代码添加 BY HANDYBY 23.06.2017 21:12:53  BEGIN
        DDYFJE    TYPE EKPO-NETWR, " 订单应付金额
        FKJD      TYPE C, " 付款节点
        YJFKRQ    TYPE EKKO-YJFKRQ1,  " 预计付款日期
        YJFKJE    TYPE EKKO-YJFKJE1,  " 预计付款金额
        FKZB      TYPE STRING  , " 付款占比
        JDYFJE    TYPE ZFI017-ZSQFKJE,  " 节点已付金额
        JDYSQJE   TYPE ZFI017-ZSQFKJE,  " 节点已申请金额
        DDKSQJE   TYPE ZFI017-ZSQFKJE,  " 订单可申请金额
        DDCEJE    TYPE ZFI017-ZSQFKJE,  " 订单超额金额
...
SELECT-OPTIONS: S_BUKRS   FOR EKKO-BUKRS NO INTERVALS NO-EXTENSION OBLIGATORY DEFAULT '1700' ,"公司代码
...
*
*  IF L_FLAG = 'X'. "励丰的单独弄
*&---码添加 BY HANDYBY 23.06.2017 15:58:45  BEGIN

*&--代码添加 BY HANDYBY 24.08.2017 15:41:14  BEGIN
  LOOP AT S_WBS.
    S_WBS-OPTION = 'CP' .
    CONCATENATE S_WBS-LOW '*' INTO S_WBS-LOW .
    MODIFY S_WBS .
  ENDLOOP.
*&--代码添加 BY HANDYBY 24.08.2017 15:41:14  END
...
*&--代码注释 BY HANDYBY 21.08.2017 16:32:45  BEGIN
*
...
*  ENDIF.
*
*&--代码注释 BY HANDYBY 21.08.2017 16:32:45  END
...
*&--代码添加 BY HANDYBY 21.08.2017 15:00:21  BEGIN
*
* 根据预计入款日期将订单分成3行
  ...
  DATA: BEGIN OF LS_ZFI017 ,
          ...
          LIFNR   TYPE ZFI017-LIFNR,
          STATU   TYPE ZFI017-STATU,
          ZCLJD   TYPE ZFI017-ZCLJD,
          FKJD    TYPE ZFI017-FKJD,
          ZSQFKJE TYPE ZFI017-ZSQFKJE,
        END OF LS_ZFI017 .
  DATA LT_ZFI017 LIKE TABLE OF LS_ZFI017 .
  DATA: BEGIN OF LS_T052U,
          ZTERM TYPE T052U-ZTERM,
          TEXT1 TYPE T052U-TEXT1,
        END OF LS_T052U .
  DATA LT_T052U LIKE TABLE OF LS_T052U .
  ...
  IF GT_DATA IS NOT INITIAL .
    ...
    SELECT ZSQD
     ...
     WHERE BUKRS = GT_DATA-BUKRS
       AND EBELN = GT_DATA-EBELN .
    SORT LT_ZFI017 BY BUKRS EBELN .
  ENDIF.

  LOOP AT GT_DATA INTO GS_DATA .

    GS_DATA-DDYFJE = GS_DATA-NETWR .
    READ TABLE LT_EKKO INTO LS_EKKO WITH KEY EBELN = GS_DATA-EBELN BINARY SEARCH .
    READ TABLE LT_T052U INTO LS_T052U WITH KEY ZTERM = LS_EKKO-ZTERM BINARY SEARCH .
    GS_DATA-ZTERM = LS_EKKO-ZTERM .
    GS_DATA-TEXT1 = LS_T052U-TEXT1 .
    CLEAR LS_EKKO .
    CLEAR LS_T052U .

    CLEAR GS_DATA-YSQJE .
    CLEAR GS_DATA-YFJE .
    READ TABLE LT_ZFI017 WITH KEY BUKRS = GS_DATA-BUKRS
                                  EBELN = GS_DATA-EBELN
                                  BINARY SEARCH TRANSPORTING NO FIELDS .
    IF SY-SUBRC = 0 .
      LOOP AT LT_ZFI017 INTO LS_ZFI017 FROM SY-TABIX .
        IF LS_ZFI017-BUKRS = GS_DATA-BUKRS AND
            LS_ZFI017-EBELN = GS_DATA-EBELN .

          IF LS_ZFI017-STATU NE ICON_DELETE .
            GS_DATA-YSQJE = GS_DATA-YSQJE + LS_ZFI017-ZSQFKJE .
          ENDIF.
          IF LS_ZFI017-ZCLJD = '3'.
            GS_DATA-YFJE = GS_DATA-YFJE + LS_ZFI017-ZSQFKJE .
          ENDIF.
          CLEAR LS_ZFI017 .
        ELSE .
          CLEAR LS_ZFI017 .
    ...
    IF GS_DATA-YFBL IS NOT INITIAL .
      IF GS_DATA-NETWR NE 0 AND GS_DATA-NETWR IS NOT INITIAL .
        GS_DATA-YFBL = GS_DATA-YFJE / GS_DATA-NETWR .
      ELSE .
        GS_DATA-YFBL = 0 .
      ENDIF.
    ELSE .
    ...
    GS_DATA-WFJE = GS_DATA-DDYFJE - GS_DATA-YFJE .

    GS_DATA-DDKSQJE = GS_DATA-DDYFJE - GS_DATA-YSQJE .
    IF GS_DATA-DDKSQJE LT 0 .
      GS_DATA-DDKSQJE = 0 .
    ENDIF.

    GS_DATA-DDCEJE = GS_DATA-YFJE - GS_DATA-DDYFJE .
    IF GS_DATA-DDCEJE LT 0 ..
      GS_DATA-DDCEJE = 0 .
    ENDIF.
    ...
    IF SY-SUBRC = 0 .
      ...
      GS_DATA-FKJD = '1' .
      GS_DATA-YJFKRQ = LS_EKKO-YJFKRQ1 .
      GS_DATA-YJFKJE = LS_EKKO-YJFKJE1 .
      IF GS_DATA-NETWR NE 0 AND GS_DATA-NETWR IS NOT INITIAL .
        GS_DATA-FKZB = LS_EKKO-YJFKJE1 / GS_DATA-NETWR .
      ELSE .
        GS_DATA-FKZB = 0 .
      ENDIF.
      READ TABLE LT_ZFI017 WITH KEY BUKRS = GS_DATA-BUKRS
      ...
      IF SY-SUBRC = 0 .
        LOOP AT LT_ZFI017 INTO LS_ZFI017 FROM SY-TABIX .
          IF LS_ZFI017-BUKRS = GS_DATA-BUKRS AND
            ...
            IF LS_ZFI017-ZCLJD = '3' AND LS_ZFI017-FKJD = '1' .
              GS_DATA-JDYFJE = GS_DATA-JDYFJE + LS_ZFI017-ZSQFKJE .
            ENDIF.
            IF LS_ZFI017-STATU NE ICON_DELETE AND LS_ZFI017-FKJD = '1' .
              GS_DATA-JDYSQJE = GS_DATA-JDYSQJE + LS_ZFI017-ZSQFKJE .
            ENDIF.
            CLEAR LS_ZFI017 .
          ELSE .
            CLEAR LS_ZFI017 .
      ...
      GS_DATA-FKJD = '2' .
      GS_DATA-YJFKRQ = LS_EKKO-YJFKRQ2 .
      GS_DATA-YJFKJE = LS_EKKO-YJFKJE2 .
      IF GS_DATA-NETWR NE 0 AND GS_DATA-NETWR IS NOT INITIAL .
        GS_DATA-FKZB = LS_EKKO-YJFKJE2 / GS_DATA-NETWR .
      ELSE .
        GS_DATA-FKZB = 1 .
      ENDIF.
      READ TABLE LT_ZFI017 WITH KEY BUKRS = GS_DATA-BUKRS
      ...
      IF SY-SUBRC = 0 .
        LOOP AT LT_ZFI017 INTO LS_ZFI017 FROM SY-TABIX .
          IF LS_ZFI017-BUKRS = GS_DATA-BUKRS AND
            ...
            IF LS_ZFI017-ZCLJD = '3' AND LS_ZFI017-FKJD = '2' .
              GS_DATA-JDYFJE = GS_DATA-JDYFJE + LS_ZFI017-ZSQFKJE .
            ENDIF.
            IF LS_ZFI017-STATU NE ICON_DELETE AND LS_ZFI017-FKJD = '2' .
              GS_DATA-JDYSQJE = GS_DATA-JDYSQJE + LS_ZFI017-ZSQFKJE .
            ENDIF.
            CLEAR LS_ZFI017 .
          ELSE .
      ...
      MOVE-CORRESPONDING LS_DATA TO GS_DATA .
      GS_DATA-FKJD = '3' .
      GS_DATA-YJFKRQ = LS_EKKO-YJFKRQ3 .
      GS_DATA-YJFKJE = LS_EKKO-YJFKJE3 .
      IF GS_DATA-NETWR NE 0 AND GS_DATA-NETWR IS NOT INITIAL .
        GS_DATA-FKZB = LS_EKKO-YJFKJE3 / GS_DATA-NETWR .
      ELSE .
        GS_DATA-FKZB = 2 .
      ENDIF.
      READ TABLE LT_ZFI017 WITH KEY BUKRS = GS_DATA-BUKRS
      ...
      IF SY-SUBRC = 0 .
        LOOP AT LT_ZFI017 INTO LS_ZFI017 FROM SY-TABIX .
          IF LS_ZFI017-BUKRS = GS_DATA-BUKRS AND
            ...
            IF LS_ZFI017-ZCLJD = '3' AND LS_ZFI017-FKJD = '3' .
              GS_DATA-JDYFJE = GS_DATA-JDYFJE + LS_ZFI017-ZSQFKJE .
            ENDIF.
            IF LS_ZFI017-STATU NE ICON_DELETE AND LS_ZFI017-FKJD = '3' .
              GS_DATA-JDYSQJE = GS_DATA-JDYSQJE + LS_ZFI017-ZSQFKJE .
            ENDIF.
            CLEAR LS_ZFI017 .
          ELSE .
  ...
  SORT GT_DATA BY EBELN FKJD .
  DELETE ADJACENT DUPLICATES FROM GT_DATA COMPARING EBELN FKJD .

  REFRESH LT_DATA .

  IF X_WFK EQ 'X' .
    DELETE GT_DATA WHERE WFJE LT 1 .
  ENDIF.
*
*&--代码添加 BY HANDYBY 21.08.2017 15:00:21  END
ENDFORM.
*&---------------------------------------------------------------------*
*&      Form  FRM_ALV_SHOW
*&---------------------------------------------------------------------*
...
*&--代码注释 BY HANDYBY 21.08.2017 11:34:30  BEGIN
  INIT_FIELDCAT 'ZTERM'          '付款条件'   '' '' '' '' '' '' ''.
  INIT_FIELDCAT 'TEXT1'          '付款条件描述'   '' '' '' '' '' '' ''.
*  INIT_FIELDCAT 'RATPZ'          '应付比例%'   '' '' '' '' '' '' ''.
*  INIT_FIELDCAT 'YFJE1'          '应付金额'   '' '' '' '' '' '' ''.
*&--代码注释 BY HANDYBY 21.08.2017 11:34:30  END

*&--代码添加 BY HANDYBY 23.06.2017 21:13:26  BEGIN
  INIT_FIELDCAT 'DDYFJE'          '订单应付金额'   '' '' '' '' '' '' ''.
  INIT_FIELDCAT 'FKJD'          '付款节点'   '' '' '' '' '' '' ''.
  INIT_FIELDCAT 'YJFKRQ'          '预计付款日期'   '' '' '' '' '' '' ''.
  INIT_FIELDCAT 'YJFKJE'          '预计付款金额'   '' '' '' '' '' '' ''.
  INIT_FIELDCAT 'FKZB'          '付款占比'   '' '' '' '' '' '' ''.
  INIT_FIELDCAT 'JDYFJE'          '节点已付金额'   '' '' '' '' '' '' ''.
  INIT_FIELDCAT 'JDYSQJE'          '节点已申请金额'   '' '' '' '' '' '' ''.
...
*&--代码添加 BY HANDYBY 21.08.2017 15:51:54  BEGIN
  INIT_FIELDCAT 'DDKSQJE'           '订单可申请金额'   '' '' '' '' '' '' ''.
  INIT_FIELDCAT 'DDCEJE'           '订单超额金额'   '' '' '' '' '' '' ''.
*&--代码添加 BY HANDYBY 21.08.2017 15:51:54  END
...
*&--代码添加 BY HANDYBY 22.08.2017 20:56:10  BEGIN
          CLEAR GS_ZFI017_1 .
*&--代码添加 BY HANDYBY 22.08.2017 20:56:10  END
  ...
  WHERE ZBOX = 'X'.
    ...
    IF GS_DATA-BXQX EQ '不保修'.
      IF NOT GS_DATA-BXQSRQ IS INITIAL.
        MESSAGE '请输入保修起始日期' TYPE 'S' DISPLAY LIKE 'E'.
        L_SUBRC = 4.
      ENDIF.
    ...
    IF GS_DATA-YJFKJE IS INITIAL .
      MESSAGE '请到对应的采购订单录入付款计划' TYPE 'S' DISPLAY LIKE 'E'.
      L_SUBRC = 4.
    ENDIF.

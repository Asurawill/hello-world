...
SELECT A~BUKRS A~GJAHR A~BELNR A~BLART A~BUDAT A~WAERS AS WAERS_1 A~BKTXT A~XBLNR A~AWTYP
     B~BUZEI B~SHKZG B~DMBTR B~WRBTR B~XNEGP B~HKONT B~ZUONR B~EBELN B~MATNR B~VBEL2 B~POSN2 B~VBUND
    INTO CORRESPONDING FIELDS OF TABLE GT_DATA
    FROM BKPF AS A
    INNER JOIN BSEG AS B
  ON A~BUKRS = B~BUKRS AND A~GJAHR = B~GJAHR AND A~BELNR = B~BELNR
  WHERE A~BUKRS = P_BUKRS AND BUDAT IN S_BUDAT  AND   ( B~HKONT  BETWEEN '8000000000' AND '8999999999' ) AND B~VBEL2 NE '' .
...
IF GT_DATA[] IS NOT INITIAL.
  ...
  SELECT * INTO CORRESPONDING FIELDS OF TABLE GT_SKAT FROM SKAT WHERE KTOPL = '1000' AND ( SAKNR  BETWEEN '8000000000' AND '8999999999' )  .

...
*PARAMETERS: p_dtl RADIOBUTTON GROUP g1  .
...
END-OF-SELECTION.
  ...
  IF <gt_show>[] IS INITIAL.
    MESSAGE s005(zfico) DISPLAY LIKE 'E'.
  ELSE.
...
* 报表行项目取值
  ...
  IF sy-subrc <> 0.
    MESSAGE S001(zfico) WITH 'ZCS001' '报表类型为1的值' DISPLAY LIKE 'E'."表 & & 没维护,请先维护好这张表!
...
* 报表行项目文本描述
  SELECT  *
    ...
    WHERE repot = g_report.
  IF sy-subrc <> 0.
    MESSAGE S001(zfico) WITH 'ZCS002' '报表类型为1的值' DISPLAY LIKE 'E'."表 & & 没维护,请先维护好这张表!
    LEAVE LIST-PROCESSING.
  ENDIF.
...
*  CHECK p_sum = 'X'.
  ...
  DATA: lt_zcs004 TYPE STANDARD TABLE OF zcs004,
        ls_zcs004 TYPE zcs004.
  ...
  LOOP AT <gt_show> ASSIGNING <gs_show>.
    ...
    CLEAR: ls_zcs004.
    ls_zcs004-rdimen = p_dimen.
    ls_zcs004-rcongr = p_congr.
    ls_zcs004-ryear = p_ryear.
    ls_zcs004-poper = p_pabrp.

    ASSIGN COMPONENT 'LINEN' OF STRUCTURE <gs_show> TO <l_linen>.
    ls_zcs004-linen = <l_linen>.

    ASSIGN COMPONENT 'ALLSUM' OF STRUCTURE <gs_show> TO <l_amoaw>.
    ls_zcs004-amoaw = <l_amoaw>.

    APPEND ls_zcs004 TO lt_zcs004.

  ENDLOOP.

  MODIFY zcs004 FROM TABLE lt_zcs004.


ENDFORM.                    " F_SAVE_ZCS004
...
FORM html_top_of_page USING document TYPE REF TO cl_dd_document.
  ...
  CONCATENATE '<HTML ><CENTER><S1><STRONG>' '<FONT SIZE=5>合并工作底稿-资产负债表</FONT>'
...
* 根据点到的行从ZCS001里面找出明细范围
  ...
  IF l_fieldname = 'GTZER'.
    SELECT  docnr
      ...
      FROM ecmca
      FOR ALL ENTRIES IN lt_click_linen
      WHERE ritem GE lt_click_linen-fitem
      AND   ritem LE lt_click_linen-titem
      AND   ryear = p_ryear

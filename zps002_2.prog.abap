...
TYPES:BEGIN OF TY_CH_SOURCE_LF,
        XH         TYPE INT4,                    "序号
...
*&--代码添加 BY HANDYBY 29.07.2017 11:54:32  BEGIN
DATA:  GT_CH_SOURCE_LF      TYPE TABLE OF TY_CH_SOURCE_LF WITH HEADER LINE.
*&--代码添加 BY HANDYBY 29.07.2017 11:54:32  END
...
PARAMETERS: P_LF   TYPE C RADIOBUTTON GROUP G1 DEFAULT 'X',               " 励丰
            P_ZT   TYPE C RADIOBUTTON GROUP G1,                           " 中天
            P_HD   TYPE C RADIOBUTTON GROUP G1,                           " 合道
            P_PR   TYPE C RADIOBUTTON GROUP G1,                           " 普瑞
            P_WK   TYPE C RADIOBUTTON GROUP G1,                           " 万科
            P_JZ   TYPE C RADIOBUTTON GROUP G1,                           " 君泽
...
FORM DOWNLOAD_TPT   USING    P_TEXT
                   VALUE(P_OBJID)
                   CHANGING PV_FILE.
  IF P_LF = 'X'.                                                        "下载 广州励丰 成本预算批导模板
    P_OBJID = 'ZLF'.
  ELSEIF P_ZT = 'X'.                                                    "下载 上海中天 合同报价金额批导模板
    P_OBJID = 'ZZT'.
  ELSEIF P_HD = 'X'.                                                   "下载 厦门合道 成本预算和合同报价批导模板
    P_OBJID = 'ZHD'.
  ELSEIF P_PR = 'X'.                                                        "下载 成都普瑞 成本预算批导模板
    P_OBJID = 'ZPR'.
  ELSEIF P_WK = 'X'.                                                    "下载 西安万科 合同报价金额批导模板
    P_OBJID = 'ZWK'.
  ELSEIF P_JZ = 'X'.                                                   "下载 长沙君泽 成本预算和合同报价批导模板
    P_OBJID = 'ZJZ'.
  ENDIF.
...
*&--代码添加 BY HANDYBY 22.05.2017 22:24:23  END
  ...
  "成本预算和合同报价的BAPI
  IF P_LF = 'X'.
    PERFORM FRM_XLS_TO_SAP TABLES GT_CH_SOURCE_LF.                           " 励丰
    PERFORM FRM_BAPI_GT_CH_OUTPUT.
  ELSEIF P_ZT = 'X'.
    PERFORM FRM_XLS_TO_SAP TABLES GT_CH_SOURCE.                          " 中天
    PERFORM FRM_BAPI_GT_CH_OUTPUT.                                       "
  ELSEIF P_HD = 'X'.
    PERFORM FRM_XLS_TO_SAP TABLES GT_CH_SOURCE.                         " 合道
    PERFORM FRM_BAPI_GT_CH_OUTPUT.                                      "
  ELSEIF P_PR = 'X'.
    PERFORM FRM_XLS_TO_SAP TABLES GT_CH_SOURCE.                           " 普瑞
    PERFORM FRM_BAPI_GT_CH_OUTPUT.                                        "
  ELSEIF P_WK = 'X'.
    PERFORM FRM_XLS_TO_SAP TABLES GT_CH_SOURCE.                          " 万科
    PERFORM FRM_BAPI_GT_CH_OUTPUT.                                       "
  ELSEIF P_JZ = 'X'.
    PERFORM FRM_XLS_TO_SAP TABLES GT_CH_SOURCE.                         " 君泽
    PERFORM FRM_BAPI_GT_CH_OUTPUT.                                      "
  ENDIF.
  "统计最后版本导入数量

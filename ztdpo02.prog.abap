...
*ENDCLASS.                    "LCL_EVENT_RECEIVER DEFINITION
  ...
  DATA:GT_PO_Z TYPE TABLE OF ZTDTZ_PO,
        GS_PO_Z TYPE ZTDTZ_PO.
...
SORT GT_PO BY PSPID POSID  TD_MATNR TD_GRPFLAG .

GT_PO_Z = GT_PO .

SORT GT_PO_Z BY PSPID POSID  EBELN EBELP.
DELETE ADJACENT DUPLICATES FROM GT_PO COMPARING  PSPID POSID  TD_MATNR TD_GRPFLAG .

   SELECT * APPENDING CORRESPONDING FIELDS OF TABLE GT_PO
     FROM  ZTDTZ_PO
     WHERE PSPID IN S_PSPID
     AND   EBELN IN S_EBELN
     AND   MATNR IN  S_MATNR
...
LOOP AT GT_DATA ASSIGNING <FS_DATA> .
  ...
  IF SY-SUBRC EQ 0 .
      ...
      WITH KEY PSPNR = GS_EKKN-PS_PSP_PNR BINARY SEARCH .
        IF SY-SUBRC EQ 0.
          "WBS号
  ...
  ENDIF.
"采购组
  READ TABLE GT_EKKO INTO GS_EKKO WITH KEY EBELN = <FS_DATA>-EBELN BINARY SEARCH .
   IF SY-SUBRC EQ 0 .
  ...
  READ TABLE GT_T024 INTO GS_T024 WITH KEY EKGRP = <FS_DATA>-EKGRP BINARY SEARCH .
    IF SY-SUBRC EQ 0 .
        <FS_DATA>-EKNAM = GS_T024-EKNAM .
    ENDIF.
"设计代码
  READ TABLE GT_ZMM024 INTO GS_ZMM024 WITH KEY MATNR = <FS_DATA>-MATNR POSID = <FS_DATA>-PSPID BINARY SEARCH.
   IF SY-SUBRC EQ 0 .
      <FS_DATA>-SJDM = GS_ZMM024-SJDM.

   ENDIF.
"WBS传送文本
  PERFORM SELXWBS USING <FS_DATA>-EBELN <FS_DATA>-EBELP CHANGING <FS_DATA>-WBSCSWB.
  ...
  READ TABLE GT_PO INTO GS_PO WITH KEY
    ...
    ELSE.
         READ TABLE GT_PO_Z INTO GS_PO_Z WITH KEY
               EBELN = <FS_DATA>-EBELN
               EBELP = <FS_DATA>-EBELP
               MATNR = <FS_DATA>-MATNR
               BINARY SEARCH .
          IF SY-SUBRC EQ 0 .
             READ TABLE GT_PO INTO GS_PO WITH KEY
              ...
              IF SY-SUBRC NE 0 .
                 "替代物料
                <FS_DATA>-TD_MATNR = GS_PO_Z-TD_MATNR . "替代物料
                <FS_DATA>-TD_MEINS = GS_PO_Z-TD_MEINS.  "替代单位
                <FS_DATA>-TD_GRPFLAG = GS_PO_Z-TD_GRPFLAG. "替代组标识
                <FS_DATA>-TD_MENGE = GS_PO_Z-TD_MENGE.    "替代数量
              "被替代物料描述
               READ TABLE GT_MAKT INTO GS_MAKT
                 WITH KEY MATNR = <FS_DATA>-TD_MATNR
                  BINARY SEARCH .

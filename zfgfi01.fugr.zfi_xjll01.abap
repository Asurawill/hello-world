...
*"  TABLES
*"      I_MANDT STRUCTURE  ZMANDT
*"      I_BUKRS STRUCTURE  ZBUKRS
*"      I_GJAHR STRUCTURE  ZGJAHR
*"      I_MONAT STRUCTURE  ZMONAT OPTIONAL
...
IF I_GJAHR[] IS NOT INITIAL.
  ...
  P_GJAHR-SIGN = 'I'.
  P_GJAHR-OPTION = 'EQ'.
  P_GJAHR-LOW = I_GJAHR-GJAHR.
  APPEND P_GJAHR.
  ENDLOOP.
  ENDIF.
IF I_MONAT[] IS NOT INITIAL.
  LOOP AT I_MONAT.
  P_MONAT-SIGN = 'I'.
  P_MONAT-OPTION = 'EQ'.
  P_MONAT-LOW = I_MONAT-MONAT.
  APPEND P_MONAT.
  ENDLOOP.
  ENDIF.
...
IF I_HKONT[] IS NOT INITIAL.
  ...
  IF  P_CHECK NE 1.
   ...
   ENDIF.
    ENDLOOP.
  ENDIF.
 IF I_RSTGR[] IS NOT INITIAL.
  LOOP AT I_RSTGR.
       CALL FUNCTION 'CONVERSION_EXIT_ALPHA_INPUT'
    EXPORTING
      INPUT  = I_RSTGR-RSTGR
    IMPORTING
      OUTPUT = I_RSTGR-RSTGR.
     TRANSLATE I_RSTGR-RSTGR  TO UPPER CASE.
     P_RSTGR-SIGN = 'I'.
     P_RSTGR-OPTION = 'EQ'.
     P_RSTGR-LOW = I_RSTGR-RSTGR.
     APPEND P_RSTGR.
    ENDLOOP.
  ENDIF.
  CLEAR O_DATA .
  ...
  FROM ZFIRFC01
  WHERE BUKRS IN P_BUKRS AND GJAHR IN P_GJAHR AND MONAT IN  P_MONAT AND KOSTL IN P_KOSTL AND KUNNR IN P_KUNNR AND LIFNR_2 IN P_LIFNR AND AUFNR IN P_AUFNR AND VBEL2_2 IN P_VBEL AND
  ...
  WHERE BUKRS = t_belen-BUKRS AND GJAHR = t_belen-gjahr AND belnr =   t_belen-belnr  AND MONAT IN  P_MONAT AND KOSTL IN P_KOSTL AND KUNNR IN P_KUNNR AND LIFNR_2 IN P_LIFNR AND AUFNR IN P_AUFNR AND VBEL2_2 IN P_VBEL AND
   HKONT IN P1_HKONT  and  blart not  in ('SA','QC')  AND RSTGR IN P_RSTGR.
  endif.
  SELECT *  APPENDING CORRESPONDING FIELDS OF TABLE T_ITEM
  FROM ZFIRFC01
  WHERE BUKRS IN P_BUKRS AND GJAHR IN P_GJAHR AND MONAT IN  P_MONAT AND KOSTL IN P_KOSTL AND KUNNR IN P_KUNNR AND LIFNR_2 IN P_LIFNR AND AUFNR IN P_AUFNR AND VBEL2_2 IN P_VBEL AND
   HKONT IN P1_HKONT  and  blart in ('SA','QC') AND RSTGR IN P_RSTGR.
...
*  FROM ZFIRFC01
*  WHERE BUKRS IN P_BUKRS AND GJAHR IN P_GJAHR AND MONAT IN  P_MONAT AND KOSTL IN P_KOSTL AND KUNNR IN P_KUNNR AND LIFNR IN P_LIFNR AND AUFNR IN P_AUFNR AND VBEL2 IN P_VBEL AND
*   HKONT IN P1_HKONT AND RSTGR IN P_RSTGR.
 ENDIF.
...
IF P2_HKONT[] IS NOT INITIAL.
  ...
  FROM ZFIRFC01
  WHERE BUKRS IN P_BUKRS AND GJAHR IN P_GJAHR AND MONAT IN  P_MONAT AND KOSTL IN P_KOSTL AND KUNNR IN P_KUNNR AND LIFNR_2 IN P_LIFNR AND AUFNR IN P_AUFNR AND VBEL2_2 IN P_VBEL AND
   HKONT like P2_HKONT-low  and  blart not  in ('SA','QC') and  RSTGR IN P_RSTGR.
  sort t_belen by bukrs gjahr belnr.
  delete adjacent duplicates from t_belen comparing bukrs gjahr belnr.
       if t_belen[] is not initial.
       SELECT * appending  CORRESPONDING FIELDS OF TABLE T_ITEM
       FROM ZFIRFC01
       for all entries in t_belen
       WHERE bukrs = t_belen-bukrs and gjahr = t_belen-gjahr AND MONAT IN  P_MONAT and belnr = t_belen-belnr  AND KOSTL IN P_KOSTL AND KUNNR IN P_KUNNR AND LIFNR_2 IN P_LIFNR AND AUFNR IN P_AUFNR AND VBEL2_2 IN P_VBEL AND
   HKONT like P2_HKONT-low  and  blart not  in ('SA','QC') and  RSTGR IN P_RSTGR...
        endif.
    SELECT *  APPENDING CORRESPONDING FIELDS OF TABLE T_ITEM
  FROM ZFIRFC01
  WHERE BUKRS IN P_BUKRS AND GJAHR IN P_GJAHR AND MONAT IN  P_MONAT AND KOSTL IN P_KOSTL AND KUNNR IN P_KUNNR AND LIFNR_2 IN P_LIFNR AND AUFNR IN P_AUFNR AND VBEL2_2 IN P_VBEL AND
    HKONT like P2_HKONT-low   and  blart in ('SA','QC') and  RSTGR IN P_RSTGR..
 sort t_item by bukrs gjahr belnr buzei.
 delete adjacent duplicates from t_item comparing bukrs gjahr belnr buzei.
     ENDLOOP.
*  P2LEN = LINES( P2_HKONT ).
...
*     FROM ZFIRFC01
*     WHERE BUKRS IN P_BUKRS AND GJAHR IN P_GJAHR AND MONAT IN  P_MONAT AND KOSTL IN P_KOSTL AND KUNNR IN P_KUNNR AND LIFNR IN P_LIFNR AND AUFNR IN P_AUFNR AND VBEL2 IN P_VBEL AND
*     HKONT LIKE P2_HKONT-LOW AND RSTGR IN P_RSTGR.
*     ENDLOOP.
  ENDIF.
IF P1_HKONT[] IS INITIAL AND P2_HKONT[] IS INITIAL.
  ...
  FROM ZFIRFC01
  WHERE BUKRS IN P_BUKRS AND GJAHR IN P_GJAHR AND MONAT IN  P_MONAT AND KOSTL IN P_KOSTL AND KUNNR IN P_KUNNR AND LIFNR_2 IN P_LIFNR AND AUFNR IN P_AUFNR AND VBEL2_2 IN P_VBEL AND
    blart not  in ('SA','QC') and  RSTGR IN P_RSTGR..
     sort t_belen by bukrs gjahr belnr.
     delete adjacent duplicates from t_belen comparing bukrs gjahr belnr.
  .
   if t_belen[] is not initial.
  ...
  WHERE bukrs = t_belen-bukrs and gjahr = t_belen-gjahr and belnr = t_belen-belnr  AND MONAT IN  P_MONAT AND KOSTL IN P_KOSTL AND KUNNR IN P_KUNNR AND LIFNR_2 IN P_LIFNR AND AUFNR IN P_AUFNR AND VBEL2_2 IN P_VBEL AND
    ...
    SELECT *  APPENDING CORRESPONDING FIELDS OF TABLE T_ITEM
  FROM ZFIRFC01
  WHERE BUKRS IN P_BUKRS AND GJAHR IN P_GJAHR AND MONAT IN  P_MONAT AND KOSTL IN P_KOSTL AND KUNNR IN P_KUNNR AND LIFNR_2 IN P_LIFNR AND AUFNR IN P_AUFNR AND VBEL2_2 IN P_VBEL
        and   blart in ('SA','QC') and  RSTGR IN P_RSTGR..
*      SELECT *  INTO CORRESPONDING FIELDS OF TABLE T_ITEM
*  FROM ZFIRFC01
*  WHERE BUKRS IN P_BUKRS AND GJAHR IN P_GJAHR AND MONAT IN  P_MONAT AND KOSTL IN P_KOSTL
*        AND KUNNR IN P_KUNNR AND LIFNR IN P_LIFNR AND AUFNR IN P_AUFNR AND VBEL2 IN P_VBEL AND RSTGR IN P_RSTGR  .
...
*  AND VBEL2 IN I_VBEL.
SORT T_ITEM BY BUKRS GJAHR MONAT BELNR BUZEI KOSTL KUNNR LIFNR AUFNR VBEL2 HKONT SHKZG RSTGR .
DELETE ADJACENT DUPLICATES FROM T_ITEM COMPARING BUKRS GJAHR MONAT BELNR BUZEI KOSTL KUNNR LIFNR AUFNR VBEL2 HKONT SHKZG RSTGR.
LOOP AT T_ITEM.

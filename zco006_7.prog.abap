...
SELECT A~BUKRS A~GJAHR A~BELNR A~BLART A~BUDAT A~WAERS AS WAERS_1 A~BKTXT A~XBLNR A~AWTYP
     B~BUZEI B~SHKZG B~DMBTR B~WRBTR B~XNEGP B~HKONT B~ZUONR B~EBELN B~MATNR B~VBEL2 B~POSN2 B~VBUND B~MENGE B~MEINS
    INTO CORRESPONDING FIELDS OF TABLE GT_DATA
    FROM BKPF AS A
    INNER JOIN BSEG AS B
  ON A~BUKRS = B~BUKRS AND A~GJAHR = B~GJAHR AND A~BELNR = B~BELNR
  WHERE A~BUKRS = P_BUKRS AND BUDAT IN S_BUDAT AND ( A~BLART NE 'WL' AND  A~BLART NE 'WA' ) AND (  ( B~HKONT  BETWEEN '6404000000' AND '6404999999' ) OR ( B~HKONT  BETWEEN '6401000000' AND '6401999999' ) ) .

SORT GT_DATA BY BUKRS GJAHR BELNR BUDAT HKONT.

IF GT_DATA[] IS NOT INITIAL.
  SELECT * INTO CORRESPONDING FIELDS OF TABLE GT_T001  FROM T001 WHERE BUKRS = P_BUKRS.
  SELECT * INTO CORRESPONDING FIELDS OF TABLE GT_SKAT FROM SKAT WHERE KTOPL = '1000' AND (  ( SAKNR  BETWEEN '6404000000' AND '6404999999' ) OR  ( SAKNR  BETWEEN '6401000000' AND '6401999999'  ) ) .
  SORT GT_SKAT BY SAKNR.
  SELECT * INTO CORRESPONDING FIELDS OF TABLE GT_VBAK FROM VBAK  ." WHERE AUART IN ('ZF2','ZOR','ZRE','ZSD') .
  SORT  GT_VBAK BY VBELN.
  SELECT * INTO CORRESPONDING FIELDS OF TABLE GT_VBAP FROM VBAP
    FOR ALL ENTRIES IN GT_DATA
    WHERE VBELN = GT_DATA-VBEL2 AND POSNR = GT_DATA-POSN2
...
FORM FRM_DEAL_DATA .
  ...
  LOOP AT GT_DATA ASSIGNING <FS_DATA> .
    ...
    IF SY-SUBRC = 0.
     ...
     ENDIF.
    "判断是否为项目订单
    IF <FS_DATA>-HKONT BETWEEN '6404010101' AND  '6404999999' .
       IF <FS_DATA>-BLART NE 'WL' AND <FS_DATA>-BLART NE 'WA'.
    ...
    "判断是否为产品订单
    IF <FS_DATA>-BLART = 'ML'.
       IF <FS_DATA>-HKONT BETWEEN  '6401000000 ' AND '6401999999'.
    ...
    ENDIF.
     IF <FS_DATA>-YWLX NE '项目' AND  <FS_DATA>-YWLX NE '产品'.
      <FS_DATA>-YWLX = '其他'.

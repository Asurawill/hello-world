...
FORM FRM_GET_DATA .
  ...
  ENDIF.
 "
  SELECT BKPF~BUKRS BKPF~GJAHR BKPF~BUDAT BKPF~XBLNR BKPF~BLART BKPF~BELNR
    BSEG~BUZEI BSEG~SHKZG BSEG~DMBTR BSEG~XNEGP BSEG~HKONT BSEG~VBEL2 BSEG~ZUONR
    ...
    WHERE BKPF~BUKRS = P_BUKRS AND BKPF~BUDAT <= P_DATE
          AND BSEG~HKONT = '1406010101'.

SORT GT_ITEM BY BUKRS BLART .

 SELECT * INTO CORRESPONDING FIELDS OF TABLE GT_VBAK
   FROM VBAK
   WHERE AUART IN ('ZF2','ZOR','ZRE','ZSD') .
...
FORM FRM_DEAL_DATA .
 ...
 LOOP AT GT_ITEM INTO GS_ITEM.
   ...
   IF GS_ITEM-BLART = 'SC'.
     GS_TOTAL-YWLX = '项目'.
     "读取销售订单
       SPLIT GS_ITEM-ZUONR  AT '/' INTO GS_TOTAL-VBELN VBELN_TMP.

   ENDIF.
  IF GS_ITEM-BLART = 'RV'.
     READ TABLE GT_VBAK WITH KEY VBELN = GS_ITEM-XBLNR BINARY SEARCH.
       IF SY-SUBRC = 0 .
         GS_TOTAL-YWLX = '产品'.
         GS_TOTAL-VBELN = GS_ITEM-XBLNR.
      ENDIF.

  ENDIF.
  IF GS_ITEM-BLART = 'WL'.
    READ TABLE GT_VBAK WITH KEY VBELN = GS_ITEM-VBEL2 BINARY SEARCH.
       IF SY-SUBRC = 0 .
         GS_TOTAL-YWLX = '产品'.
         GS_TOTAL-VBELN = GS_ITEM-VBEL2.

        ENDIF.
  ENDIF.
  IF GS_ITEM-BLART = 'ML'.
    GS_TOTAL-YWLX = '其他-材料差异'.
   ENDIF.
 IF GS_TOTAL-YWLX NE '其他-材料差异' AND GS_TOTAL-YWLX NE '项目' AND GS_TOTAL-YWLX NE '产品' .
   GS_TOTAL-YWLX = '其他'.
 ENDIF.
 "公司代码
 ...
 ENDLOOP.
 SORT GT_TOTAL BY YWLX VBELN.
 DELETE GT_TOTAL WHERE  QCYE = 0 AND BQJF = 0 AND  BQDF = 0.
 LOOP AT GT_TOTAL INTO GS_TOTAL WHERE VBELN NE ''.
   "项目名称读取
   "项目名称

...
*&--代码添加 BY HANDYBY 07.06.2017 21:26:57  BEGIN
           9029 NAME P9029 .
*&--代码添加 BY HANDYBY 07.06.2017 21:26:57  END

DATA:MASSN_LEN TYPE I .
TYPES:
  BEGIN OF GST_DATA ,
    ...
    MNTXT       TYPE MNTXT, "操作类型_文本
    MASSG       TYPE MASSG, "操作原因
    MGTXT       TYPE MGTXT, "操作原因
    MASSN_DATE  TYPE SY-DATUM, "开始日期
...
*&--代码添加 BY HANDYBY 07.06.2017 21:27:43  BEGIN
    ZZBZ0       TYPE P9029-ZZBZ0,
    ZZBZ1       TYPE P9029-ZZBZ1,
    ZZBZ2       TYPE P9029-ZZBZ2,
    ZZBZ3       TYPE P9029-ZZBZ3,
*&--代码添加 BY HANDYBY 07.06.2017 21:27:43  END
...
DATA: L_ENDDA TYPE SY-DATUM."离职事件的结束日期
DATA:LZ_P0001 TYPE P0001 OCCURS 0 WITH HEADER LINE. "LZ_P0001存储离职P0001信息类型数据。
...
*&---------------------------------------------------------------------*
INITIALIZATION.
*IF S_DATE-HIGH NE '00000000' OR ( S_DATE-HIGH > SY-DATUM ) .
...
START-OF-SELECTION.
  "
  CHECK MASSN_LEN NE 0.

  GET PERAS .
  PERFORM SEL_DATA."查询数据
...
FORM FRM_ADD_FIELDCATALOG .
  ...
  PERFORM FRM_FILL_CAT USING:
      ...
      18 '' 'PERSK_PTEXT' '员工子组',
       ...
       39 '' 'DATD3' '社会工龄起算日期',
       40 '' 'DATD2' '累计司龄起算日期',
       41 '' 'DATD1' '当前司龄起算日期（入职日期）' ,
       42 '' 'DATD4' '实际离职日期' ,
       42 '' 'CTTYP_CTTXT' '当前合同类型' ,
       43 '' 'HTBEGDA' '当前合同起始日期' ,
       44'' 'CTEDT' '当前合同结束日期' ,
       45 '' 'ZHTDQYF' '距当前合同到期月份数' ,
       46 '' 'ZHTDQTX' '合同到期提醒' ,
       47 '' 'ZZHTCS' '自2008年后合同签订次数' ,
       48 '' 'ZZHTGS_TXT' '合同公司主体' ,
       49 '' 'ZZQSRQ' '当前合同签署日期' ,
       50 '' 'ZZQSBZ' ' 当前合同签署备注' ,
       51 '' 'ZZSYQ' '试用期截止日' ,
       52 '' 'ZZSYQDQ' '距试用期到期天数' ,
       53 '' 'ZZZTX' '转正提醒' ,
       54 '' 'ZZMYSTS' '距满月考核到期天数' ,
       55'' 'ZZMYTX' '满月提醒' ,
       56 '' 'ZZGSSJHM' '公司手机号' ,
       57 '' 'ZZGSDH' ' 公司短号' ,
       58 '' 'ZZGRSJHY' '手机号码一' ,
       ...
       64 '' 'ZZGRYX' '个人邮箱' ,
       65 '' 'ZZDYXM' '第一紧急联系人姓名' ,
       66 '' 'ZZDYGX' '第一紧急联系人与本人关系' ,
       67 '' 'ZZDYDZ' '第一紧急联系人地址' ,
       68'' 'ZZDYHM' '第一紧急联系人手机号码' ,
      69 '' 'ZZXZ_TXT' '星座' ,
       70 '' 'ZZXX_TXT' '血型' ,
       71 '' 'NAME2' '英文名' ,
       72 '' 'ZZZJXY' '宗教信仰' ,
...
*&--代码添加 BY HANDYBY 07.06.2017 21:28:36  BEGIN
      75 '' 'ZZBZ0' '工资成本归属单位' ,
     76 '' 'ZZBZ1' '备注2' ,
     77 '' 'ZZBZ2' '备注3' ,
     78 '' 'ZZBZ3' '备注4' .
*&--代码添加 BY HANDYBY 07.06.2017 21:28:36  END
...
FORM CHECK_CHOOSE .
  IF S_DATE-HIGH EQ '00000000'.
    MESSAGE '时间段最大值不能为空' TYPE 'E'  .
  ENDIF.
  DESCRIBE TABLE PNPMASSN LINES MASSN_LEN.
  IF MASSN_LEN = 0 .
    MESSAGE '请选择一项操作类型！' TYPE 'E'  .
  ENDIF.
  IF MASSN_LEN > 1.
    MESSAGE '请只选择一项操作类型查询！' TYPE 'E'  .
  ENDIF.
ENDFORM.
...
FORM SEL_DATA."查询数据 .
  CLEAR:P0000,P0001,P0002,P0185,P0041,P0534,P9006,P9022,P0016,P3211,P0009,P9105.
  CLEAR :GS_DATA .
  IF S_DATE IS NOT INITIAL.
    "  RP_PROVIDE_FROM_LAST P0000 P0000-SUBTY  PN-BEGDA PN-ENDDA  ."S_DATA-LOW S_DATA-HIGH.
  ENDIF.
...
*ENDPROVIDE.
  ...
  ENDIF.
  IF PNPMASSN[] IS NOT INITIAL.
    DELETE P0000[] WHERE MASSN NOT IN PNPMASSN .
  ENDIF.
  IF PNPMASSG[] IS NOT INITIAL.
    DELETE P0000[] WHERE MASSG NOT IN PNPMASSG .
  ENDIF.
  ...
  ENDIF.
  IF S_DATE IS NOT INITIAL. "以时间段HIEA-HIGH.
    " DELETE P0000[] WHERE ENDDA < S_DATE-HIGH.
    DELETE P0001[] WHERE BEGDA > S_DATE-HIGH.
    DELETE P0001[] WHERE ENDDA < S_DATE-HIGH.
    " DELETE P0000[] WHERE BEGDA > S_DATE-HIGH.
    "  DELETE P0000[] WHERE ENDDA < S_DATE-HIGH.
    DELETE P0002[] WHERE BEGDA > S_DATE-HIGH..
    DELETE P0002[] WHERE ENDDA < S_DATE-HIGH.
    DELETE P0185[] WHERE BEGDA > S_DATE-HIGH.
    DELETE P0185[] WHERE ENDDA < S_DATE-HIGH.
    DELETE P0534[] WHERE BEGDA > S_DATE-HIGH.
    DELETE P0534[] WHERE ENDDA < S_DATE-HIGH.
    DELETE P9006[] WHERE BEGDA > S_DATE-HIGH.
    DELETE P9006[] WHERE ENDDA < S_DATE-HIGH.
*  DELETE P9022[] WHERE BEGDA > P_DATE.
*  DELETE P9022[] WHERE ENDDA < P_DATE.
    DELETE P0016[] WHERE BEGDA > S_DATE-HIGH.
    DELETE P0016[] WHERE ENDDA < S_DATE-HIGH.
    DELETE P3211[] WHERE BEGDA > S_DATE-HIGH.
    DELETE P3211[] WHERE ENDDA < S_DATE-HIGH.
    DELETE P0009[] WHERE BEGDA > S_DATE-HIGH.
    DELETE P0009[] WHERE ENDDA < S_DATE-HIGH.
    DELETE P0031[] WHERE BEGDA > S_DATE-HIGH.
    DELETE P0031[] WHERE ENDDA < S_DATE-HIGH.
    DELETE P9379[] WHERE BEGDA > S_DATE-HIGH.
    DELETE P9379[] WHERE ENDDA < S_DATE-HIGH.

*&--代码添加 BY HANDYBY 07.06.2017 21:29:12  BEGIN
    DELETE P9029[] WHERE ENDDA < S_DATE-HIGH.
    DELETE P9029[] WHERE ENDDA < S_DATE-HIGH.
*&--代码添加 BY HANDYBY 07.06.2017 21:29:12  END
...
*     EXIT.  "排除离职 员工
*   ENDIF.
    READ TABLE PNPMASSN WITH KEY LOW = P0000-MASSN.
    IF SY-SUBRC NE 0.
      CONTINUE.
    ENDIF.
    IF S_DATE-LOW <= P0000-BEGDA AND P0000-BEGDA <=  S_DATE-HIGH.
      " GS_DATA-ZZXH = NINDEX."组织序号
      GS_DATA-PERNR = PERAS-PERNR. "员工编号
      GS_DATA-MASSN = P0000-MASSN."操作类型
      GS_DATA-MASSG = P0000-MASSG."操作原因
      GS_DATA-MASSN_DATE = P0000-BEGDA ."操作开始日
      SELECT SINGLE MNTXT INTO GS_DATA-MNTXT FROM T529T WHERE  SPRSL = '1' AND  MASSN = P0000-MASSN   . "操作类型_文本
      SELECT SINGLE MGTXT INTO GS_DATA-MGTXT FROM T530T WHERE  SPRSL = '1' AND  MASSN = P0000-MASSN AND MASSG = P0000-MASSG."操作原因——文本
  ...
  LOOP AT P0001 .
    CLEAR L_ENDDA .
    " IF S_DATA-HIGH >= P0000-BEGDA AND S_DATA-HIGH <= P0000-ENDDA.
...
*        AND   ENDDA = '99991231'..
      ...
      ENDLOOP.
    ELSE.
      CLEAR :LZ_P0001 ,LZ_P0001[].
      DATA: T_PERNR TYPE PRELP-PERNR.
      T_PERNR = GS_DATA-PERNR.
      L_ENDDA = P0001-BEGDA - 1 . "离职事件的前一记录的结束日期
...
* Implement suitable error handling here
      ENDIF.
      IF T_SUBRC  EQ 0 .
        READ TABLE LZ_P0001 INDEX 1 .
        IF SY-SUBRC = 0 .
          GS_DATA-PLANS = LZ_P0001-PLANS.
          SELECT SINGLE PLSTX INTO GS_DATA-PLANS_PLSTX
            FROM T528T
            WHERE  SPRSL = '1'
            AND OTYPE = 'S'
...
*       IM_VIEW_KOKRS       =
        IM_OBJID = OBJID
    "   IM_ISTAT = ' '
        IM_BEGDA = S_DATE-HIGH
        IM_ENDDA = S_DATE-HIGH
      IMPORTING
        SHORT    = P_SHORT
        LONG     = P_LONG.
    IF P_SHORT NE '' .
...
* IMPLEMENT SUITABLE ERROR HANDLING HERE
  ...
  LOOP AT P0041.
    CASE P0041-DAR01 .
      ...
      WHEN 'D4'.
        GS_DATA-DATD4 = P0041-DAT01."实际离职日期
    ENDCASE.
    CASE P0041-DAR02 .
      WHEN 'D1' .
      ...
      WHEN 'D4'.
        GS_DATA-DATD4 = P0041-DAT02."实际离职日期
    ENDCASE.
    CASE P0041-DAR03 .
      WHEN 'D1' .
      ...
      WHEN 'D4'.
        GS_DATA-DATD4 = P0041-DAT03."实际离职日期
    ENDCASE.
    CASE P0041-DAR04 .
      WHEN 'D1' .
      ...
      WHEN 'D4'.
        GS_DATA-DATD4 = P0041-DAT04."实际离职日期
    ENDCASE.
...
*&--代码添加 BY HANDYBY 07.06.2017 21:29:46  BEGIN
  LOOP AT P9029.
  ...
  ENDLOOP.
*&--代码添加 BY HANDYBY 07.06.2017 21:29:46  END
